<template>
  <div class="eventJK_commonMD">
    <div class="title"><span>处置方事件评价差评率排名</span></div>
    <div class="putcontent">
      <div class="echarts_box">
        ...
        />
        <div class="tip5lv">
          <div class="tt">评价差评率</div>
          <div class="rankbox">
            <div v-for="(val, idx) in rankList" :key="idx" class="lan">{{ val.poorRatingRate }}%</div>
          </div>
        </div>
      </div>
    </div>
...
import { chapinglvFiveOptions } from './echartsOptions'
  ...
  methods: {
    async getMdDataFn(sysCodes, dateArr) {
      ...
      if (res.code === SUCCESS_CODE) {
        const useEchartsOptions = chapinglvFiveOptions
        ...
        const qushiList = res.data?.differenceList || []
        this.rankList = qushiList
        qushiList.forEach(val => {
          const eobj1 = {
            value: val.differenceCount,
            name: val.sysName,
            count: val.differenceCount
          }
          const eobj2 = {
