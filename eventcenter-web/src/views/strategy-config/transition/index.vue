      ...
      <div slot="content" class="result-content">
        ...
        <div v-loading="loading" class="result-list" :style="resultStyle">
          <el-table ref="tableRef" :data="dataList" style="width: 100%" tooltip-effect="light">
            ...
            <el-table-column :width="100" fixed="right" class-name="table-column-action">
              <template #header>
                <span>操作</span>
              </template>
              <template v-slot="{ row }">
...
export default {
  ...
  methods: {
    ...
    // 删除
    _del(row) {
      this.$confirm('确定要删除该转换配置吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        customClass: 'cestc-confirm-warning'
      }).then(() => {
        moduleSrv.busValuesConfigDelete(row).then((res) => {
          if (res.code === SUCCESS_CODE) {
            this.$message({
              message: '操作成功！',
              type: 'success'
    ...
    // 启用、停用状态
    _changeStatus(row) {
      moduleSrv.busValuesConfigUpdate(row).then((res) => {
        if (res.code === SUCCESS_CODE) {
          this.$message({
            message: '操作成功！',
            type: 'success'
