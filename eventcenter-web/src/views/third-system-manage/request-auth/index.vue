  ...
  <div class="page-wrapper">
    <common-page-list has-footer>
      <search
        slot="search"
        ref="searchRef"
        :event-platform-options="eventPlatformOptions"
            ...
            <template v-for="(item, index) in tableColumn">
              ...
              >
                ...
                <template v-slot="{ row, column }">
                  <span v-if="item.prop == 'status'" class="switch-status">
                    <el-switch
...
export default {
  ...
  created() {
    this.getEventPlatformOptions()
    this._search()
  },
  methods: {
    async getEventPlatformOptions() {
      const res = await moduleSrvWhh.getPlatformInfoSelect()
      this.eventPlatformOptions = res.data.map(e => ({ dictValue: e.platformCode, dictLabel: e.platformName, dictDesc: e.platformName }))
    },
    copy() {
