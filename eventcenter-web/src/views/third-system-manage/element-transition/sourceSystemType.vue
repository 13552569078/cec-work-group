            ...
            <div slot="content" class="result-content">
              ...
              <div v-loading="loading" class="result-list" :style="resultStyle">
                ...
                >
                  ...
                  <el-table-column prop="sourceSysEventTypeFullName" label="来源系统类型名称" show-overflow-tooltip>
                    <template slot-scope="scope">
                      <span>{{ scope.row.sourceSysEventTypeFullName || scope.row.sourceSysEventTypeName }}</span>
                    </template>
                  </el-table-column>
                  <el-table-column prop="sourceSysEventTypeCode" label="来源系统类型编码" width="200" />
                  <el-table-column prop="thisSysEventTypeName" label="本系统类型名称" show-overflow-tooltip>
                    <template slot-scope="scope">
                      <el-cascader
                        v-if="scope.row.isEdit"
                        ref="thisSysEventTypeRef"
                        v-model="scope.row.thisSysEventTypeCode"
                        :options="options"
                        placeholder="请选择本系统类型名称"
                        style="width: 300px;"
                        :props="{ label: 'name', value: 'code' }"
                        @change="handleChange"
                      />
                      <span v-else>{{ scope.row.thisSysEventTypeFullName || scope.row.thisSysEventTypeName }}</span>
                    </template>
                  </el-table-column>
                  <el-table-column prop="thisSysEventTypeCode" label="本系统类型编码" width="200">
                    <template slot-scope="scope">
                      <span>{{ scope.row.thisSysEventTypeCode && Array.isArray(scope.row.thisSysEventTypeCode) ? scope.row.thisSysEventTypeCode[scope.row.thisSysEventTypeCode.length - 1]: scope.row.thisSysEventTypeCode }}</span>
                    </template>
                  </el-table-column>
...
import { disabledNode } from '@/utils/treeUtil'
...
export default {
      ...
      searchFilterItems: [
        {
          type: 'input',
          label: '来源系统类型名称',
          labelKey: 'sourceSysEventTypeName',
          options: [],
          value: '',
          defaultValue: '',
          placeholder: '请输入内容'
        },
        {
          type: 'input',
          label: '来源系统类型编码',
          labelKey: 'sourceSysEventTypeCode',
          options: [],
          value: '',
          defaultValue: '',
          placeholder: '请输入内容'
  ...
  async mounted() {
    ...
    const { data } = await eventTypeModuleSrv.postEventTypeselectAllTree({})
    disabledNode(data)
    this.options = data
  },
  methods: {
    _search() {
      ...
      this.loading = true
      moduleSrv.sourceSystemRelationList(params).then((res) => {
    ...
    handleEdit(index, row, isEdit) {
      ...
      if (isEdit) {
        console.log('getCodes', this.getCodes(this.options, row.thisSysEventTypeCode))
        if (row.thisSysEventTypeCode) row.thisSysEventTypeCode = this.getCodes(this.options, row.thisSysEventTypeCode)?.reverse()
        this.$set(row, 'isEdit', isEdit)
      } else {
        if (row.thisSysEventTypeCode) {
          moduleSrv.saveSourceSystemRelation(
            this.currentSelectedOption ? {
              ...row,
              thisSysEventTypeCode: row.thisSysEventTypeCode[row.thisSysEventTypeCode.length - 1],
              thisSysCode: this.currentSelectedOption.sourceSystemCode,
              thisSysEventTypeId: this.currentSelectedOption.id,
              thisSysEventTypeName: this.currentSelectedOption.name
            } : { ...row, thisSysEventTypeCode: row.thisSysEventTypeCode[row.thisSysEventTypeCode.length - 1] }).then((res) => {
            if (res.code === 0) {
              this.$message({
                message: '保存成功！',
                type: 'success'
