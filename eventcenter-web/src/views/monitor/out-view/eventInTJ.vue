<template>
  <div class="eventJK_commonMD">
    <div class="title"><span>事件流出监控统计</span></div>
    <div class="putcontent">
      <div class="totalbox">
        <div class="e">
          <div class="l"><i class="icon1" /><span>统计周期事件流出量：</span></div>
          <div class="r"><b>{{ allDataObj.withinCount | changeNumToStr }}</b><span>件</span></div>
        </div>
        <div class="e">
          <div class="l"><i class="icon2" /><span>今日事件流出总量：</span></div>
          <div class="r"><b>{{ allDataObj.todayCount | changeNumToStr }}</b><span>件</span></div>
        </div>
        <div class="e">
          <div class="l"><i class="icon3" /><span>历史事件流出总量：</span></div>
...
export default {
  ...
  methods: {
    async getMdDataFn(dateArr) {
      ...
      const res = await moduleSrv.getOutflowTotalCount(params)
