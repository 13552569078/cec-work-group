<template>
  <div class="eventJK_commonMD">
    <div class="title"><span>任务类型统计</span></div>
    <div class="putcontent">
      <div class="echarts_box">
        ...
        <div class="topbox">
          ...
          <div v-for="(val, idx) in top5List" :key="idx" class="e"><div><i :class="['n'+(idx+1)]" /><span>{{ val.eventTypeName }}</span></div><b>{{ val.count | changeNumToStr }}</b></div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    async getMdDataFn(dateArr) {
      ...
      const res = await moduleSrv.getOutflowByDealEventType(params)
