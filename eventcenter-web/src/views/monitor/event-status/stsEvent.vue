<template>
  <div class="eventstatuswraper">
    <div class="statuscontentbox">
      <div class="toparea">
        <div class="cmtitle">
          <div class="tl">事件状态监控</div>
          <div class="rl">
        ...
        <div class="platformbox">
          ...
          <div class="platformboxinner">
            <div class="modulebox">
              <!-- TODO SYM getEventThirdStateSummary:需要后端反【已办结】、【流转异常】、【处置异常】、【退回发起方】 -->
              <div v-for="(val, idx) in eventThirdStateList" :key="idx" class="emdl" :class="{'selected': val.eventThirdStateCode === curEventThirdStateCode}" @click="eventThirdStateClickFn(val.eventThirdStateCode)">
                <div class="t"><i :class="['pticon' ,'pticon'+(idx+1)]" /><span>{{ val.eventThirdStateName }}</span></div>
                <div class="m">
          ...
          <div class="platformboxinner">
            <div class="modulebox">
              <div v-for="(val, idx) in sysList" :key="idx" class="emdl" :class="{'selected': val.sysCode === curSysCode}" @click="sysClickFn(val.sysCode)">
                <div class="t"><span class="paiming">{{ 'NO.'+(idx+1) }}</span><span>{{ val.sysName }}</span></div>
                <div class="m">
                  <div class="d1">事件数量</div>
                  <div class="d2"><b>{{ val.count | changeNumToStr }}</b><span>件</span></div>
                </div>
        ...
        <div class="cmtitle">
          ...
          <div class="serchlistipt">
            ...
            <el-input v-model="searchEventCode" :placeholder="conditionType===1?'请输入事件编号':'请输入发起方事件编号'" class="input-with-select" clearable>
              <el-button slot="append" icon="el-icon-search" @click="getEventThirdStateList" />
            </el-input>
          </div>
        </div>
                  ...
                  <div class="expandcolumnbox">
                    <div v-if="row.expandTaskList && row.expandTaskList.length > 0" class="epdcblinebox">
                      <div v-for="(val, idx) in row.expandTaskList" :key="idx" class="epdcbline">
                        ...
                        <!-- TODO SYM -->
                        <div class="txt wid3">处置超时：{{ val.overtimeFlag !== '' ? eventOverTimeOptions.get(val.overtimeFlag) : '-' }}</div>
...
export default {
  ...
  methods: {
    async quickDateCk(type) {
      ...
      await this.getEventThirdStateSummary()
      if (this.curEventThirdStateCode) {
        this.curSysCode = ''
        await this.getEventThirdStateBySys()
      }
      await this.getEventThirdStateList()
    },
    async eventThirdStateClickFn(eventThirdStateCode) {
      this.curSysCode = ''
      this.sysList = []
      if (this.curEventThirdStateCode === eventThirdStateCode) {
        this.curEventThirdStateCode = ''
      } else {
        this.curEventThirdStateCode = eventThirdStateCode
        await this.getEventThirdStateBySys()
      }
      await this.getEventThirdStateList()
    },
    ...
    async getEventThirdStateSummary() {
      const params = {
        beginTime: this.curDateArr[0],
        endTime: this.curDateArr[1]
      }
      const res = await moduleSrv.getEventThirdStateSummary(params)
      if (res.code === SUCCESS_CODE) {
        this.eventThirdStateList = res.data?.eventThirdStateList || []
        this.eventTotal = res.data?.eventCount || 0
      }
    },
    async getEventThirdStateBySys() {
      const params = {
        beginTime: this.curDateArr[0],
        endTime: this.curDateArr[1],
        eventThirdState: this.curEventThirdStateCode
      }
      const res = await moduleSrv.getEventThirdStateBySys(params)
      if (res.code === SUCCESS_CODE) {
        this.sysList = res.data || []
      }
    },
    async getEventThirdStateList() {
      const params = {
        beginTime: this.curDateArr[0],
        endTime: this.curDateArr[1],
        eventThirdState: this.curEventThirdStateCode || undefined,
      ...
      const res = await moduleSrv.getEventThirdStateList(params)
      if (res.code === SUCCESS_CODE) {
    ...
    async clickexpandFn(row, expandedRows) {
      ...
      const res = await moduleSrv.getEventThirdStateByEventCode(params)
      if (res.code === SUCCESS_CODE) {
            ...
            .emdl {
              ...
              .t {
                ...
                i {
                  ...
                  height: 16px;
                  &.pticon {
                    background: url('~@/assets/workbench/evtsicon1@2x.png') no-repeat;
                    background-size: 100% 100%;
                  }
                  &.pticon1 {
                    background: url('~@/assets/workbench/evtsicon1@2x.png') no-repeat;
                  ...
                  &.pticon3 {
                    background: url('~@/assets/workbench/evtsicon2@2x.png') no-repeat;
                    background-size: 100% 100%;
                  }
                  &.pticon4 {
                    background: url('~@/assets/workbench/evtsicon3@2x.png') no-repeat;
                    background-size: 100% 100%;
                  }
                }
                span {
