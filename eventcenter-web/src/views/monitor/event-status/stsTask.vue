      ...
      <div class="toparea">
        ...
        <div class="platformbox">
          ...
          <div class="platformboxinner">
            <div class="modulebox">
              <!-- TODO SYM getTaskStateSummary:需要后端反【退回总线】、【处置超时】 -->
              <div v-for="(val, idx) in taskThirdStateList" :key="idx" class="emdl" :class="{'selected': val.code === curTaskThirdStateCode}" @click="taskThirdStateClickFn(val.code)">
                <div class="t"><i :class="['pticon' ,'pticon'+(idx+1)]" /><span>{{ val.name }}</span></div>
                <div class="m">
        ...
        <div class="cmtitle">
          ...
          <div class="serchlistipt">
            ...
            <el-input v-model="searchEventCode" :placeholder="conditionType===1?'请输入事件编号':'请输入发起方事件编号'" class="input-with-select" clearable>
              <el-button slot="append" icon="el-icon-search" @click="getTaskStateList" />
            </el-input>
          </div>
        </div>
                  ...
                  <div class="expandcolumnbox">
                    <div v-if="row.expandTaskList && row.expandTaskList.length > 0" class="epdcblinebox">
                      <div v-for="(val, idx) in row.expandTaskList" :key="idx" class="epdcbline">
                        <!-- <div class="txt wid1">{{ val.overStatusText === '派发异常' ? '接收方系统：' : (val.overStatusText === '超期' ? '接收方系统：' : (val.overStatusText === '反馈异常' ? '发起方系统：' : '')) }}{{ val.sysName }}</div>
                        <div class="txt wid2">任务流转状态：<span>{{ val.overStatusText || '-' }}</span></div> -->
...
export default {
  ...
  methods: {
    async quickDateCk(type) {
      ...
      await this.getTaskStateSummary()
      if (this.curTaskThirdStateCode) {
        this.curSysCode = ''
        await this.getTaskStateBySys()
      }
      await this.getTaskStateList()
    },
    async taskThirdStateClickFn(taskThirdStateCode) {
      this.curSysCode = ''
      this.sysList = []
      if (this.curTaskThirdStateCode === taskThirdStateCode) {
        this.curTaskThirdStateCode = ''
      } else {
        this.curTaskThirdStateCode = taskThirdStateCode
        await this.getTaskStateBySys()
      }
      await this.getTaskStateList()
    },
    ...
    async getTaskStateSummary() {
      const params = {
        beginTime: this.curDateArr[0],
        endTime: this.curDateArr[1]
      }
      const res = await moduleSrv.getTaskStateSummary(params)
      if (res.code === SUCCESS_CODE) {
        this.taskThirdStateList = res.data?.taskStateList || []
        this.taskTotal = res.data?.taskCount || 0
      }
    },
    async getTaskStateBySys() {
      const params = {
        beginTime: this.curDateArr[0],
        endTime: this.curDateArr[1],
        taskState: this.curTaskThirdStateCode
      }
      const res = await moduleSrv.getTaskStateBySys(params)
      if (res.code === SUCCESS_CODE) {
    ...
    async getTaskStateList() {
      const params = {
        beginTime: this.curDateArr[0],
        endTime: this.curDateArr[1],
        taskState: this.curTaskThirdStateCode || undefined,
      ...
      const res = await moduleSrv.getTaskStateList(params)
      if (res.code === SUCCESS_CODE) {
    ...
    async clickexpandFn(row, expandedRows) {
      ...
      // const res = await moduleSrv.getTaskStateByEventCode(params)
            ...
            .emdl {
              ...
              .t {
                ...
                i {
                  ...
                  height: 16px;
                  &.pticon {
                    background: url('~@/assets/workbench/taksicon1@2x.png') no-repeat;
                    background-size: 100% 100%;
                  }
                  &.pticon1 {
                    background: url('~@/assets/workbench/taksicon1@2x.png') no-repeat;
