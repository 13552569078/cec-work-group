<template>
  <div class="eventJK_commonMD">
    <div class="title"><span>接入平台</span></div>
    <div class="putcontent">
      <div class="echarts_box">
        ...
        <div class="topbox">
          ...
          <div v-for="(val, idx) in top5List" :key="idx" class="e"><div><i :class="['n'+(idx+1)]" /><span>{{ val.sysName }}</span></div><b>{{ val.count | changeNumToStr }}</b></div>
        </div>
      </div>
    </div>
  </div>
...
import { inPlatformOptions } from './echartsOptions'
...
export default {
  ...
  methods: {
    async getMdDataFn(dateArr) {
      ...
      const res = await moduleSrv.getInflowAnalysisBySys(params)
      if (res.code === SUCCESS_CODE) {
        const qushiList = res.data || []
        // top5数据处理
        this.top5List = qushiList.slice(0, 5)
        // echarts数据处理
        const useEchartsOptions = inPlatformOptions
        ...
        if (qushiList.length > 5) {
          ...
          qushiList.slice(5).forEach(val => {
            other.value += Number(val.percent)
            other.radio += Number(val.percent)
            other.count += Number(val.count)
          })
