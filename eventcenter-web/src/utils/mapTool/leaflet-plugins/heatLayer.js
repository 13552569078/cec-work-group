...
!(function() {
  ...
  (t.prototype = {
    ...
    gradient: function(t) {
      ...
      var s = a.createLinearGradient(0, 0, 0, 256);
...
(L.HeatLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  _redraw: function() {
    ...
    var m = new L.Bounds(L.point([-_, -_]), l.add([_, _]))
    var c = void 0 === this.options.max ? 1 : this.options.max
    var u =
        void 0 === this.options.maxZoom
          ? this._map.getMaxZoom()
          : this.options.maxZoom
    var f = 1 / Math.pow(2, Math.max(0, Math.min(u - this._map.getZoom(), 12)))
    var g = _ / 2
    var p = []
    var v = this._map._getMapPanePos()
    var w = v.x % g
    var y = v.y % g
