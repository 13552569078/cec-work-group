<template>
  <div class="process-time-line">
    ...
    <div class="content mt20">
      <el-timeline>
        <el-timeline-item v-for="(item, index) in processList" :key="index" placement="top">
          <!-- dot -->
          <template slot="dot">
          ...
          <div :class="['content', item.collapse ? 'is-collapse' : 'is-expend']">
            <ul class="head">
              ...
              <li class="name">{{ _transformName(item) }}</li>
            ...
            <div v-show="!item.collapse" class="expend">
              ...
              <div v-if="item.showMore" class="info-item-wrap m-t-16">
                <div class="comment comment1">
                  <label class="title">处理人</label>
                  <p>{{ item.dealUserName }}</p>
                </div>
                <div class="comment comment1">
                  <label class="title">处理人电话</label>
                  <p>{{ item.dealUserPhone }}</p>
                </div>
              </div>
              <div v-if="item.showMore" class="info-item-wrap m-t-16">
                <div class="comment comment1">
                  <label class="title">超时状态</label>
                  <p>{{ item.overTimeFlagName }}</p>
                </div>
                <div class="comment comment1">
                  <!-- <label class="title">主处置部门</label>
                  <p>{{ item.responsibleDepartment }}</p> -->
                </div>
              </div>
              <div class="comment">
                ...
                <ul class="media-wrapper">
                  <li v-for="media in item.media" :key="media.id" class="media-item">
                    <video
                      ...
                      class="video"
                    />
                    <el-image v-else style="width: 100%; height: 100%" :src="media.mediaPath" />
                    <!-- hover 操作 -->
                    <div class="hover">
                      <div class="hover-wrapper">
                        <span
                          v-if="media.mediaType.includes('audio')"
                          class="icon-radio icon"
                          title="音频播放"
                          @click.stop="_openDialog(media)"
                        />
                        <span
                          v-if="media.mediaType.includes('video')"
                          class="icon-video icon"
                          title="视频播放"
                          @click.stop="_openDialog(media)"
                        />
                        <span
                          v-if="media.mediaType.includes('image')"
                          class="icon-search icon"
                          title="查看"
                          @click.stop="_openDialog(media, item)"
                        />
                        <!-- 三方先隐藏下载 -->
                        <span v-show="false" class="icon-download icon" title="下载" @click.stop="_download(media)" />
                      </div>
                    </div>
                  </li>
                </ul>
    ...
    <el-image-viewer v-if="showPreview" :url-list="imgList" :on-close="() => showPreview = false" />
    <!-- 视频播放弹窗 -->
    <video-dialog :visible.sync="videoDialog" :src="videoSrc" :type="videoType" />
    <!-- 城管处理轨迹弹窗 -->
    <chengguan-dialog :visible.sync="chengguanDialog" :process-list="chengGuanList" />
  </div>
</template>
<script>
