...
class CAxios {
  ...
  constructor(axiosConfig = { timeout: 100000 } as CAxiosConfig) {
    if (axiosConfig.serverConfig && typeof axiosConfig.serverConfig === 'object') {
      // const responseJson = {
      //   'TITLE': "常州视频管理",
      //   'PROD_PREFIX': "video-list",
      //   'BASE_URL_API': "https://onenet.jscz.org.cn:8088/safety/midscr/zdywtg/zdpc",
      //   'BASE_URL_VIDEO': "/swes/fx/video"
      // };
      ...
      xhr.onreadystatechange = () => {
        if (xhr.status === 200 && xhr.readyState === 4) {
          ...
          document.title = responseJson.TITLE || '模板项目';
    ...
    this.service.interceptors.response.use(
      response => {
        ...
        if (String(responseCode) !== '0' && String(responseCode) !== '200') {
