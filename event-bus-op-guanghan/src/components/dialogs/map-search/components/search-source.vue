<template>
  ...
  <c-tabs v-model="searchParams.type"  :tabs="SEARCH_SOURCE_TYPE_OPTION" @click="getList" />
  ...
  <hl-group  v-if="list.length !== 0" dir="vertical" full>
    ...
    <hl-group dir="horizontal" class="items-middle m-t-lg items-between page-container">
      <div>资源数/<span class="page-count">{{ pagination.total }}</span>个</div>
...
const fetchData = async () => {
  const res = await api.postResourcelistPage(props.keyword, searchParams.type ? [searchParams.type] : [], pagination.pageSize, pagination.currentPage);
