<template>
  <common-dialog title="下发指令" width="calc(var(--xl) * 20)" @close="handleCloseAllEventDialog">
    <hl-form :model="form" :rules="rules" :label-position="['items-left', 'items-top']" item-gap="var(--md)" ref="formRef">
      <hl-form-item label="指令标题" prop="title">
        <hl-input v-model="form.title" placeholder="请输入指令标题" class="w-full" show-word-limit maxlength="50" />
      </hl-form-item>
      <hl-form-item prop="users" label="接收人">
        <org-user-item v-model="form.users" tip="接收人" :currentUserIsDefaultCheck="false" />
      </hl-form-item>
      <hl-form-item label="指令内容" prop="content">
        <hl-input v-model="form.content" native-type="textarea" :rows="3" block placeholder="请输入指令内容" show-word-limit maxlength="500" />
      </hl-form-item>
    ...
    <hl-group gap="var(--sm)" align="items-right" class="w-full m-t-lg">
      <hl-button effect="light" @click="handleCloseAllEventDialog">取消</hl-button>
...
const { handleCloseAllEventDialog, currentEventId, currentEventCode } = useGlobalState();
