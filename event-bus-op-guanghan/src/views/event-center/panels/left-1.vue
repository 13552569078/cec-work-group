<template>
  <hv-panel :title="optionConfig.cardTitle">
    <hl-group full="full-x" dir="vertical" gap="var(--xxs)" style="height: calc(var(--xl) * 7.4)">
      <!-- :chart-data="chartData" -->
      <platform-statistics-bar :chart-data="chartData" :title="optionConfig.chartsTitle"
        :doneLabel="optionConfig.doneLabel" :unDoneLabel="optionConfig.unDoneLabel"> </platform-statistics-bar>
    </hl-group>
  </hv-panel>
</template>
...
import { apiIndexPageObj, apiIndexPageWaitingObj, apiIndexPagePendingObj, apiIndexPageFinishedObj } from '@/apis';
...
const getAllData = async (params: any) => {
  ...
  const { data } = await apiIndexPageObj.postPlatform({ districtCode: params.district, startTime, endTime });
...
const getWaitingData = async (params: any) => {
  const { data } = await apiIndexPageWaitingObj.postPlatform({ ...params, districtCode: params.district, platformCode: params.platform });
  chartData.value = data.map(item => {
    return {
      platform: item.platformName,
