<template>
  <hv-panel title="事件列表">
    <!--仅 ALL 展示实时事件 -->
    <template #title v-if="searchParams.eventStatus === ALL">
      <hl-group align="items-middle">
        <div class="header-tabs" :class="headerMode === 'realTime' ? 'active' : null" @click="headerMode = 'realTime'">实时事件</div>
        <div class="header-tabs" :class="headerMode === 'eventList' ? 'active' : null" @click="headerMode = 'eventList'">事件列表</div>
      </hl-group>
    </template>
    ...
    <template v-if="searchParams.eventStatus === ALL && headerMode === 'realTime'">
      <c-tabs :isFull="true" :tabs="realTimeTabs" class="w-full m-t-sm m-b-xs" v-model="realTimeeventTabType"></c-tabs>
      <real-time-event-list></real-time-event-list>
    </template>
    <template v-else>
        ...
        <c-tabs :isFull="true" v-model="eventTabType" :tabs="eventTabOptions">
          <template v-slot="{ option }">
            <span class="standard-text">
              {{ option.label }}
              <b style="color: #82ffa1">{{ statNum && !isNaN(statNum[option.numName]) ? statNum[option.numName] : '--' }}</b> 件
            </span>
          </template>
        </c-tabs>
      </hl-group>
      <template v-if="EVENT_ORIGIN_TABS.includes(eventTabType)">
        <origin-event-list @setTabsNum="setTabsNum" />
      </template>
...
const realTimeTabs = [
  ...
  {
    label: '一般事件',
    value: 'realTimeNormal'
  },
  {
    label: '紧急事件',
    value: 'realTimeEmergency'
  }
];
const realTimeeventTabType = ref('realTimeAll');
