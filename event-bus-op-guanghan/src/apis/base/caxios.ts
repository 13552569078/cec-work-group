...
// const serverConfig = require('../../../public/config/config');

// 不需要noUseFor
const noUseFor = ['/monitorRegion'];
...
class CAxios {
  ...
  constructor(axiosConfig = { timeout: 100000 } as CAxiosConfig) {
    if (process.env.NODE_ENV !== 'development') {
      Object.assign(axiosConfig.serverConfig, window.serverConfig);
    }
    ...
    this.service.interceptors.response.use(
      response => {
        ...
        if (responseCode && String(responseCode) !== '200') {
  ...
  handleRequestConfig(config: AxiosRequestConfig, url: string) {
    ...
    const useFor = document.location.href.indexOf('/alarm/') > -1 ? 1 : 2;
    if (/\/monitor\/|\/warning\//.test(document.location.href) && noUseFor.some(item => url?.indexOf(item) === -1)) {
  ...
  handleTokenExpired(response: AxiosResponse) {
...
const HongLingAxios = new CAxios({
  ...
  arrayBufferUrls,
  formDataUrls,
  serverConfig: process.env.NODE_ENV !== 'development' ? GlobalAPI.config : undefined,
}).service;
