...
function initTreeLi(json){
	...
	if(json.type == "user"){
		...
		var img = $("<img/>").css({"float": "left","margin": "2px 6px 0 0 "}).data("info", json.info).attr("src", 'data:image/png;base64,'+imgMap["select"]).width(16).height(16);
		...
		var img = $("<img/>").css({"float": "left","margin": "2px 6px 0 0 "}).addClass("expend").attr("src", 'data:image/png;base64,'+imgMap["flod"]).width(16).height(16).css({"cursor": "pointer"});
	        ...
	        $.each(res.lstTalkbackInfo,function(i,n){
	        	var trRow = $("<tr/>").append($("<td/>").html(n.strTalkbackName)).append($("<td/>").html(n.strStartTime)).append($("<td/>").html(n.nTalkbackID)).append($("<td/>").html(n.nTimeDuration)).append($("<td/>").html(meetingStauts[n.nStatus])).append($("<td/>").append(function(){
			           ...
			           return del;
		           		...
		           		var div = $("<div/>").css({"margin-left": "20px","overflow-y": "auto"}).height("100%").width("100%").append(table1)
		           			getTalkrecordList({
		           				"strDomainCode":res.strDomainCode,
		           				"nTalkbackID": n.nTalkbackID}, table1);
	           				recordTr.append(div)
	...
	if(sdk.sie.userStatus == 1)
	{
		...
		root.append(
			$("<div/>").attr("id","createTrunkparam").append(
				...
				$("<div/>").height("50").append($("<label/>").html("是否录音：").css({"float":"left","margin-top": "10px"})).append(
					$("<input/>").attr({"id": "c-nRecordStatus", type: "checkbox"}).width(16).height(16).css({"float":"left","margin-top": "12px","cursor":"pointer"})
				)
			).append(
