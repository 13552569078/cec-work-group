...
const chartData1 = ref<{ value: any; label: string }[]>([]); // 高频事件--一般事件
const chartData4 = ref<{ value: any; label: string }[]>([]); // 高频事件--紧急事件
...
const getData = async (params: any) => {
  if (searchParams.platform === EVENT_PLARFORM_CODE) {
    ...
    chartData4.value = [];
    // 高频事件--一般事件
    const res1 = await apiIndexPageObj.postEvents({
      ...params,
      districtCode: params.district,
      platformCode: params.platform,
      eventUrgent: 1,
    });
    chartData1.value = res1.data.map(item => {
      return {
    ...
    // 高频事件--紧急事件
    const res4 = await apiIndexPageObj.postEvents({
      ...params,
      districtCode: params.district,
      platformCode: params.platform,
      eventUrgent: 2,
    });
    chartData4.value = res4.data.map(item => {
      return {
        label: item.eventTypeName,
        value: item.eventNum,
        precent: item.eventRate,
...
const clickBar = async (barItem: any) => {
  handleListDialogParams.value = {
    ...
    eventStatus: '',
    eventUrgent: barItem.eventUrgent,
  };
