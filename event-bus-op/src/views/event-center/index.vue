<template>
  ...
  >
    <template v-if="!isEventSearchMode">
      <template v-if="searchParams.eventStatus === ALL">
        <right-1 ref="rightFirstRef" />
        <right-2 ref="rightSecondRef" :eventStatus="searchParams.eventStatus" />
      </template>
      ...
      <template
        v-if="
          searchParams.eventStatus === PENDING || searchParams.eventStatus === FINISHED
        "
      >
