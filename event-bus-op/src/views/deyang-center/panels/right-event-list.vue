<template>
  <hv-panel title="事件列表">
    ...
    <template v-if="eventTabType === 'manual'">
      <origin-event-list ref="originEventList" :params="eventSearchParams" :autoTrigger="false" height="calc(var(--xl) * 28.85)" @update="handleUpdate"/>
    </template>
    <template v-else>
      <self-handle-event-list ref="selfHandleEventList" :params="eventSearchParams" :autoTrigger="false" height="calc(var(--xl) * 28.85)" @update="handleUpdate" />
    </template>
