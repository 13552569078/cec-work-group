<template>
    <hl-group style="height: calc(var(--xxs) * 75.75); position: relative" full="full-x">
      <hl-button
        v-if="showBack"
        type="primary"
...
const { searchParams, currentEventId, handleShowEventDetail } = useGlobalState();
...
const getTopicEvent = async () => {
  let params;
  if (props.recent20Flag) {
    params = {
      flag: props.recent20Flag,
  ...
  const categories: GraphChartCategories[] = [];
  if (res.data?.length) {
...
const getEventKeyWords = async (data: {
  name: string;
  value: string;
  eventId: string;
}) => {
