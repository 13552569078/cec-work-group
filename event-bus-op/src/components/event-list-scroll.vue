  ...
  <hl-scrollbar :style="`height: ${height};`" class="m-t-xs">
    ...
    >
      <hl-group
        ...
        :key="index"
        class="event-list w-full"
        :class="platTypeStyle === 'other' ? 'no-line' : ''"
        dir="vertical"
        style="position: relative"
      >
        <hl-group
          class="event-top cursor-pointer"
          align="items-between"
        ...
        <hl-group
          class="event-middle m-t-xs p-l-md p-r-md cursor-pointer"
          align="items-middle"
        >
          <hl-popover
            placement="bottom"
          ...
          <hl-group align="items-middle">
            <hl-popover
              ...
              :disabled="item.address?.length < 18"
              trigger="hover"
              popper-style="--popover-bg-color:rgba(14,23,38,0.96);"
            >
              <template #reference>
        ...
        <!-- 暂时去掉定位功能-end -->
        <hl-group
          v-if="platTypeStyle === 'mix'"
          class="event-bottom full p-l-md p-r-md"
          align="items-middle"
        >
          <hl-group class="plat-start static" align="items-left">
            ...
            >
              <template #reference>
                <span class="bottom-count ellipsis cursor-pointer">{{
                  item.value1
                }}</span>
              </template>
            </hl-popover>
          ...
          <hl-group class="plat-end" align="items-left">
            <hl-group class="bottom-title m-r-xs" align="items-middle"
              >接收平台:</hl-group
            >
            <hl-group class="plat-list" align="items-middle">
              <div class="p-l-xs p-r-xs" v-if="item.value2?.length === 0">--</div>
              <div
                class="plat-normal p-l-xs p-r-xs"
                v-else-if="item.value2?.length === 1"
              >
                {{ item.value2[0] }}
              </div>
                ...
                >
                  <template #reference>
                    <hl-group class="circle-list">
                      <hl-group
                        class="plat-normal plat-circle cursor-pointer"
                        v-for="ele in item.value2.slice(0, platSize)"
                        :key="ele"
                        align="items-center"
                      >
                        {{ ele.slice(0, 1) }}
                      ...
                      <hl-group
                        v-if="item.value2?.length > platSize"
                        align="items-middle"
                      >
                        ···
                      </hl-group>
          ...
          <hl-group class="overtime-area" align="items-right">
            ...
            <hl-group align="items-center">
              <hl-group
                v-for="(ele, index) in item.overtimeList"
                :key="ele.name + index"
                align="items-middle"
              >
                <hl-group align="items-middle">
                ...
                <hl-group
                  ...
                  align="items-middle"
                  :style="{
                    'background-image': `url(${require('@/assets/images/eventCenter/overtime_' +
                      ele.value +
                      '.png')}`,
                  }"
                >
        ...
        >
          <hl-group class="plat-start static" align="items-left">
            ...
            >
              <template #reference>
                <span class="simple-bottom-count ellipsis cursor-pointer">{{
                  item.value1
                }}</span>
              </template>
            </hl-popover>
          ...
          <hl-group class="plat-end" align="items-left">
            ...
            >
              <template #reference>
                <span class="simple-bottom-count-mid ellipsis cursor-pointer">{{
                  item.value2
                }}</span>
              </template>
            </hl-popover>
        ...
        <hl-group
          v-if="platTypeStyle === 'progress'"
          style="position: absolute; bottom: calc(var(--xl) * 0.5); width: 100%"
        >
          <progress-bar :percent="item.confidence" :progress-name="progressName" />
        </hl-group>
        <hl-group
          v-if="platTypeStyle === 'other'"
          class="p-l-md p-r-md p-t-md"
          style="position: absolute; bottom: calc(var(--xl) * 0.3); width: 100%"
          align="items-between"
        >
          ...
          <hl-group align="items-middle" gap="var(--sm)">
            ...
            <span
              :class="[
                'standard-text',
                item.overtimeFlag !== null && item.overtimeFlag === 1
                  ? 'text-primary'
                  : 'text-danger',
              ]"
              >{{
                item.overtimeFlag === null
                  ? "--"
                  : item.overtimeFlag === 1
                  ? "未超时"
                  : "已超时"
        ...
        <hl-group
          v-if="platTypeStyle === 'realTime'"
          class="p-l-md p-r-md p-t-md"
          style="position: absolute; bottom: calc(var(--xl) * 0.3); width: 100%"
          align="items-between"
        >
          <hl-group align="items-middle" gap="var(--sm)">
            <span class="standard-text">紧急程度</span>
            <span
              :class="{
                'no-urgent': item.eventUrgent === '1',
                'real-urgent': item.eventUrgent === '2',
              }"
              >{{ item.eventUrgentText }}</span
            >
          </hl-group>
          <hl-group align="items-middle" gap="var(--sm)">
            <span class="standard-text">汇聚状态</span>
            <span class="standard-text">{{ item.convergeStateText }}</span>
          </hl-group>
          <hl-group align="items-middle" gap="var(--sm)">
            <span class="standard-text">事件状态</span>
            <event-status-tag
              v-if="item.showSelfHandle"
              :event-status="item.eventStatus"
            />
            <event-status-tag v-else :event-status="item.eventStatus" />
          </hl-group>
        </hl-group>
      </hl-group>
    </vue-seamless-scroll>
...
import { GlobalAPI, MAP_POINT } from '@/common';
...
const defaultOption = {
  step:
    window.screen.height > 2500
      ? GlobalAPI.config.LIST_SCROOL_STEP_BIG || 15
      : GlobalAPI.config.LIST_SCROOL_STEP_NORMAL || 3,
  limitMoveNum: 4,
};
...
.event-list {
  ...
  .real-urgent {
    padding: 0 calc(var(--xl) * 0.16);
    background: rgba(240, 37, 75, 0.24);
    font-size: calc(var(--xl) * 0.58);
    font-family: PingFangSC-Medium, PingFang SC;
    font-weight: 500;
    color: #fd6161;
