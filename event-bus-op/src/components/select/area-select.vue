<template>
  <hl-selector class="areaSelector" :class="!isFullWidth ? '' : 'w-full'" ref="selector1" filterable clearable block
    :popper-class="!isFullWidth ? 'poper-width' : ''" @input="handleTreeInput" @clear="handleClear" @resize="resize"
    placeholder="请选择行政区划" popper-append-to-body size="small" style="--input-padding-x: 0px">
    <hl-scrollbar>
      <hl-tree ref="hlTreeSingle" :style="treeStyle" node-key="id" :data="areaList" :filter-node-method="filterNode"
        class="p-md small-text" @node-click="clickTree" :props="defaultProps" default-expand-all />
    </hl-scrollbar>
  </hl-selector>
</template>
...
const clickTree = (data: ItreeNode) => {
  ...
  searchParams.districtName = areaName;
  emit('change', data);
};
const reset = () => {
