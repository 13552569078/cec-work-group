<template>
  <hl-group
    :class="['layer-switcher-box', isSingleEvent ? 'layer-switcher-single-box' : '', !isFooterEventListOpen ? 'lower' : '']"
    align="items-bottom"
  >
    <hl-group
      v-show="isOpen"
      class="layer-panel m-r-md static p-md"
      dir="vertical"
      gap="var(--xs)"
    >
        ...
        <hl-group>
          ...
          <hl-group gap="var(--sm)" align="items-middle">
            <div class="switch" @click="handleToggleMapRect">
              <img
                :src="!rectMapVisible ? switchLeft : switchRight"
      ...
      <hl-group class="layer-box" dir="vertical">
        <hl-checkbox-group
          v-model="layerSwitch.layerSelected"
          :dir="isSingleEvent ? 'vertical' : ''"
      ...
      <hl-group>
        <div class="text map-level-title m-r-sm">紧急程度</div>
        <!-- <c-tabs
          v-model="layerSwitch.eventUrgent"
          :tabs="eventUrgentOptions"
          mini
          @click="(eventUrgent) => getAggregationPoint({ ...searchParams, eventUrgent })"
        /> -->
        <hl-select
          size="sm"
          class="w-xs"
          v-model="layerSwitch.eventUrgent"
          @change="(eventUrgent) => getAggregationPoint({ ...searchParams, eventUrgent })"
        >
          <hl-option label="全部" value=""></hl-option>
          <hl-option label="一般" value="1"></hl-option>
          <hl-option label="紧急" value="2"></hl-option>
        </hl-select>
      </hl-group>
      ...
      <hl-group align="items-between items-middle">
        ...
        <hl-button
          type="primary"
          size="sm"
          @click="openLayerSearch('资源搜索', 'searchSource')"
          >资源搜索</hl-button
        >
      </hl-group>
      <hl-group class="layer-box" dir="vertical">
        <hl-checkbox-group
          v-model="layerSwitch.resourcesSelected"
          :dir="isSingleEvent ? 'vertical' : ''"
          class="checkbox-group"
          gap="var(--xs)"
          :style="isSingleEvent ? '' : 'flex-wrap: wrap;'"
          align="items-between"
        >
          <hl-checkbox
          ...
          >
            ...
            <img class="img-checked" :src="checked" />
            <img
              class="img-icon m-l-xs m-r-xs"
              :src="sanxiao"
              v-if="`${item.type}` === RESOURCE.SANXIAO"
            />
            <img
              class="img-icon m-l-xs m-r-xs"
              :src="business"
              v-if="`${item.type}` === RESOURCE.BUSINESS"
            />
            <img
              class="img-icon m-l-xs m-r-xs"
              :src="traffic"
              v-if="`${item.type}` === RESOURCE.TRAFFIC"
            />
            <img
              class="img-icon m-l-xs m-r-xs"
              :src="hospital"
              v-if="`${item.type}` === RESOURCE.HOSPITAL"
            />
            <img
              class="img-icon m-l-xs m-r-xs"
              :src="school"
              v-if="`${item.type}` === RESOURCE.SCHOOL"
            />
            <img
              class="img-icon m-l-xs m-r-xs"
              :src="hotel"
              v-if="`${item.type}` === RESOURCE.HOTEL"
            />
            <img
              class="img-icon m-l-xs m-r-xs"
              :src="supply"
              v-if="`${item.type}` === RESOURCE.SUPPLY"
            />
            <span>{{ item.name }}</span>
          </hl-checkbox>
        </hl-checkbox-group>
      ...
      <hl-group align="items-between items-middle">
        ...
        <hl-button
          type="primary"
          size="sm"
          @click="openLayerSearch('视频搜索', 'searchVideo')"
          >视频搜索</hl-button
        >
      </hl-group>
      <hl-group class="layer-box" dir="vertical">
        <hl-checkbox-group
          v-model="layerSwitch.videosSelected"
          dir="vertical"
          class="checkbox-group"
          gap="var(--xs)"
        >
    ...
    <hl-group dir="vertical" gap="var(--sm)">
      ...
      <img
        class="switcher cursor-pointer static"
        :src="isOpen ? eventSearchBtnOn : eventSearchBtn"
        v-if="showMapResources"
        @click="toggleEventSearchMode"
      />
      <img
        class="switcher cursor-pointer static"
        :src="eventGuardBtn"
      />
      <!-- <img
        class="refresh cursor-pointer static"
        v-if="['event-center'].includes(routerName as string)"
        :src="indexPageRefreshFlag ? refreshBtnOn : refreshBtn"
        @click="handleRefresh"
      />
      <img
        class="measure cursor-pointer static"
        :src="isMeasureMode ? measureBtnOn : measureBtn"
        @click="handleMeasure"
      /> -->
    ...
    <!-- 弹窗：地图搜索 -->
    <map-layer-search
      v-if="maplayerSearchVisible"
      :title="layerTitle"
      :searchMode="layerSearchMode"
      @close="handleClose"
    />
  </hl-group>
</template>
...
import { MAP_AGGRE, MAP_HEAT, MAP_POINT, RESOURCE_TYPE as RESOURCE, GlobalAPI } from '@/common';
import { useGlobalState, useMapTools, useMapData, useRefresh } from '@/hooks';
...
import refreshBtn from '@/assets/images/layerSwitcher/refresh.png';
// import refreshBtnOn from '@/assets/images/layerSwitcher/refresh-on.png';
import refreshBtnOn from '@/assets/images/layerSwitcher/refresh.png';
...
import eventSearchBtn from '@/assets/images/layerSwitcher/buttonIcons/icon_search.png';
import eventSearchBtnOn from '@/assets/images/layerSwitcher/buttonIcons/icon_search_on.png';
import eventGuardBtn from '@/assets/images/layerSwitcher/buttonIcons/icon_guard.png';
import eventGuardBtnOn from '@/assets/images/layerSwitcher/buttonIcons/icon_guard_on.png';
...
import supply from '@/assets/images/layerSwitcher/supply.png';
...
const { refreshIndexData, indexPageRefreshFlag } = useRefresh();
const { getAggregationPoint } = useMapData();
...
const handleMeasure = () => {
  measureUtil.value && measureUtil.value.changeMode('length', {
    ...
    lineColor: MAP_CONFIG.lineColor,
  });
...
.refresh {
  display: block;
  width: calc(var(--xxs) * 40/4);
  height: calc(var(--xxs) * 40/4);
}
