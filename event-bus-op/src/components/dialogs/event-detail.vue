<template>
  <common-dialog title="事件详情" width="calc(var(--xl) * 29.58)" @close="$emit('close')">
    <template #right>
      <hl-button class="standard-text handle-button"  @click.stop="toggleInstructDialog" >下发指令</hl-button>
      <hl-button class="standard-text handle-button m-r-xs" @click.stop="goDetail">进入事件</hl-button>
    </template>
