<template>
  <hl-group  dir="vertical" class="list-cell w-full p-r-sm" >
    ...
    <hl-group class="event-top" align="items-between">
      <hl-group align="items-middle items-center m-t-xxs" style="height: calc(var(--xl));">
        ...
        <hl-popover placement="bottom" :content="data.name" :disabled="data.name?.length < 22" trigger="hover" popper-style="--popover-bg-color:rgba(14,23,38,0.96);">
          <template #reference>
    ...
    <hl-group class="p-sm" align="items-between">
      ...
      <span v-else class="display-flex items-middle">
        <img class="status-icon" :src="data.status===1?online:''" v-if="data.status"/>
        {{ data.statusName }}
      </span>
...
const { layerSwitch } = useGlobalState();
