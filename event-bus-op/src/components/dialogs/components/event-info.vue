<template>
  <hl-group v-if="data" dir="vertical" gap="var(--xxs)">
    <hl-group class="box-styled-short p-xs p-l-lg p-t-sm" dir="vertical" gap="var(--xxs)">
      <span class="gradient-text standard-text" style="font-weight: normal !important;">{{ data.title }}</span>
    ...
    <hl-group class="bg-cell p-xs" align="items-between">
      ...
      <event-status-tag :event-status="data.eventStatus"/>
    </hl-group>
    <hl-group class="bg-cell p-xs" align="items-between">
      <span class="label">超时状态</span>
      <span
        :class="['standard-text', data.overtimeFlag !== null && data.overtimeFlag === 1 ? 'text-primary' : 'text-danger']">{{
    ...
    <hl-group class="bg-cell p-xs" align="items-between">
      ...
      <span class="standard-text">{{ handleEmptyStr(data.eventSourceName) }}</span>
    </hl-group>
    <hl-group class="bg-cell p-xs" align="items-between">
    ...
    <hl-group class="bg-cell p-xs" align="items-between" full dir="vertical">
    ...
    <hl-group class="bg-cell p-xs" gap="var(--md)">
      ...
      <hl-thumb :src="img" size="xxxl" :preview-src-list="srcList"/> -->
        <hl-thumb v-for="img in data.beforeMedia" :key="img.mediaName" :src="img.mediaPath" size="xxxl"
    ...
    <hl-group class="bg-cell p-xs" gap="var(--md)">
      ...
      <hl-group gap="var(--xs)">
        <!-- <hl-thumb :src="img" size="xxxl" :preview-src-list="srcList"/>
      <hl-thumb :src="img" size="xxxl" :preview-src-list="srcList"/> -->
        <hl-thumb v-for="img in data.afterMeida" :key="img.mediaName" :src="img.mediaPath" size="xxxl"
...
const beforeImgList = computed(() => {
  if (props.data.beforeMedia.length === 0) return [];
  return props.data.beforeMedia.map((image: Image) => {
    return image.mediaPath;
  });
});

const afterImgList = computed(() => {
  if (props.data.afterMeida.length === 0) return [];
  return props.data.afterMeida.map((image: Image) => {
    return image.mediaPath;
  });
});
</script>
