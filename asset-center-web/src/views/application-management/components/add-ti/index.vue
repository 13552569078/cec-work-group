  ...
  <div style="width:100%;height:100%">
    ...
    >
      <template v-slot:content>
        <detailBaseInfo v-if="pageStatus=='detail'" hide-scene :form-data="formData" @showLog="showLog" />
        <div v-show="pageStatus!='detail'" class="white-box-wrap">
          <div ref="base-info">
            <c-form
              ref="dataForm"
              :model="formData"
                  ...
                  <c-col :span="span">
                    ...
                    >
                      <c-input
                        ...
                        type="textarea"
                        class="form-input-item"
                        :placeholder="pageStatus === 'detail'?'':'请输入请求参数实例'"
                        :autosize="pageStatus === 'detail'?true:false"
                        maxlength="500"
                        :rows="4"
                        resize="none"
        ...
        <div v-if="pageStatus === 'detail'&& formData.status===5" class="white-box-wrap nopadding">
...
import { tagTree, assetCodeExist, assetNameExist, apiAssetListPage, assetReferenced } from '@/api/index'
import { citysignsAdd, citysignsDetail, citysignsUpdate } from '@/api/statistics-asset'
...
export default {
  ...
  data() {
    return {
      ...
      pageTypeList: window.__systemConfig && window.__systemConfig.bizType && window.__systemConfig.bizType.tzList && window.__systemConfig.bizType.tzList.map(item => {
      ...
      rules: {
        businessType: [
          { required: true, message: '请选择体征类型', trigger: 'blur' }
        ],
        loginType: [
  ...
  computed: {
    pageTitle() {
      const PAGE_STATUS_OBJ = { edit: '编辑接口', add: '新增接口', detail: '查看接口' }
  ...
  created() {
    ...
    if (this.pageStatus === 'detail' || this.pageStatus === 'edit') { this.initData() } else {
      console.log(this.formData.subtype, 'this.formData.subtype')
  ...
  methods: {
    ...
    initData() {
      // 获取详情
      citysignsDetail(this.$route.query.id).then(res => {
    ...
    handleOperate(item) {
      if (item.label === '更多') {
        this.$router.go(-1)
        openAssetEditorUrl({ ...this.formData, assetType: 1, subtype: '14', readonly: this.pageStatus === 'detail' ? 1 : '' }, this.$message)
    ...
    saveForm() {
      this.$refs.dataForm.validate((valid) => {
        ...
        if (this.pageStatus === 'add') {
          citysignsAdd({ ...this.formData, referAssetList, referPrivateAssetList, referShareAssetList, assetCode: (this.formData.assetCodePrefix || '') + this.formData.assetCode }).then(res => {
            if (res.code * 1 === 0) {
              this.$message.success(res.msg)
              this.$router.go(-1)
              openAssetEditorUrl({ ...this.formData, assetCode: (this.formData.assetCodePrefix || '') + this.formData.assetCode, assetType: 1, subtype: '14' }, this.$message)
            }
          })
        } else {
          citysignsUpdate({ ...this.formData, referAssetList, referPrivateAssetList, referShareAssetList, assetCode: (this.formData.assetCodePrefix || '') + this.formData.assetCode }).then(res => {
            if (res.code * 1 === 0) {
              this.$message.success(res.msg)
              this.$router.go(-1)
              openAssetEditorUrl({ ...this.formData, assetCode: (this.formData.assetCodePrefix || '') + this.formData.assetCode, assetType: 1, subtype: '14' }, this.$message)
