...
<template>
  <div class="add-app-wrap">
    <c-form ref="dataForm" :model="formData" label-width="100px" class="cls-model-form"
      ...
      <c-form-item
        v-if="deployType!=1"
        class="half-form-item w600" label="是否关联注册应用" prop="appLinke" :disabled="flag === 'detail'">
        <template slot="label">
          <span>
            是否关联注册应用
        ...
        <c-radio-group v-model="formData.appLinke" disabled>
      ...
      <c-form-item disabled class="half-form-item"
                   label="发布端" prop="appType">
        <c-checkbox-group v-model="formData.appType" disabled>
          ...
          <div v-for="(path, i) in initImgList" :key="`initImg-${i}`" class="avatar-wrap avatar"
            ...
            <span class="cover">
              ...
              <!-- <i
                class="icon-cover"
                :class="{ active: i === selectImgIndex }"
                @click.stop="handlePictureSelect(i)"
              /> -->
            </span>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        appLinke: [
          { required: true, message: '请选择是否关联注册应用', trigger: 'blur' }
        ],
        shopCode: [
        ...
        appName: [
          ...
          //     // console.log(val, 'kdkdkdkkd')
          //     // if (!/\w{1,20}/.test(val)) {
          //     //   return cb('应用名称只支持字母或数字或下划线或汉字')
          //     // }
          //     // 不能重名, 复制、导入不进行重名校验
          //     // if (this.flag === 'copy' || this.flag === 'import') {
          //     //   return cb();
          //     // }
  ...
  methods: {
    setAppData() {
      ...
      this.formData.sceneId = findOne.packSceneId
    ...
    initData(id) {
      appDetail(id).then((res) => {
        const data = res.data;
        if (!data) return;
        data.appLinke = data.shopCode ? 1 : 2
        console.log(data, '===========')
    ...
    setFormData(data) {
      ...
      this.formData.appType = data.appType && data.appType.map(item => item + '') || [];
      this.formData.sceneId = data.sceneId;
      this.formData.sceneName = data.sceneName;
      this.formData.appDesc = data.appDesc;
      this.formData.appLinke = data.appLinke;
      this.formData.appIcon = data.appIcon;
      this.formData.shopCode = data.shopCode;
      this.formData.shopName = data.shopName;
      this.formData.tagNameList = data.tagList ? data.tagList.map(d => d.tagName) : [];

      this.formData.appCode = data.appCode;
      ...
      const imgIsInit = this.initImgList.findIndex(
        (d) => d === this.formData.appIcon
      );
      console.log(data.appIcon, this.initImgList)
      console.log(imgIsInit, 'imgIsInit')
      if (imgIsInit === -1) {
        this.selectImgIndex = 999;
        this.appIcon = this.formData.appIcon;
      } else {
        this.selectImgIndex = imgIsInit;
      }
    },
      ...
      setTimeout(
        () =>
          this.$refs.dataForm.validate((valid) => {
            ...
            const sendFn = appUpdate

            sendFn({ ...this.formData, appType: this.formData.appType.map(item => (+item)), coverConfigJson: JSON.stringify(this.formData.coverConfigJson) }).then((res) => {
              if (res.code * 1 === 0) {
    ...
    saveAndImport() {
      ...
      this.$confirm(
        ...
        .then(() => {
          ...
          importAppItem(this.formData).then((res) => {
            if (res.code * 1 === 0) {
