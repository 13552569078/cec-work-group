...
import { modeAdd, modeDetail, modeUpdate } from '@/api/'
...
export default {
  ...
  data() {
    return {
      ...
      pageTypeList: window.__systemConfig && window.__systemConfig.bizType && window.__systemConfig.bizType.modelList && window.__systemConfig.bizType.modelList.map(item => {
      ...
      rules: {
        ...
        publisher: [
          { required: true, message: '请输入发布方', trigger: 'blur' }
        ],
        version: [
          {
            required: true,
  ...
  methods: {
    ...
    initData() {
      // 获取详情
      modeDetail(this.$route.query.id).then(res => {
    ...
    handleOperate(item) {
      if (item.label === '更多') {
        this.$router.go(-1)
        openAssetEditorUrl({ ...this.formData, assetType: 7, subtype: '72', readonly: this.pageStatus === 'detail' ? 1 : '' }, this.$message)
    ...
    saveForm() {
      this.$refs.dataForm.validate((valid) => {
        ...
        if (this.pageStatus === 'add') {
          modeAdd({ ...this.formData, referAssetList, referPrivateAssetList, referShareAssetList, assetCode: (this.formData.assetCodePrefix || '') + this.formData.assetCode }).then(res => {
            if (res.code * 1 === 0) {
              this.$message.success(res.msg)
              this.$router.go(-1)
              openAssetEditorUrl({ ...this.formData, assetCode: (this.formData.assetCodePrefix || '') + this.formData.assetCode, assetType: 7, subtype: '72' }, this.$message)
            }
          })
        } else {
          modeUpdate({ ...this.formData, referAssetList, referPrivateAssetList, referShareAssetList, assetCode: (this.formData.assetCodePrefix || '') + this.formData.assetCode }).then(res => {
            if (res.code * 1 === 0) {
              this.$message.success(res.msg)
              this.$router.go(-1)
              openAssetEditorUrl({ ...this.formData, assetCode: (this.formData.assetCodePrefix || '') + this.formData.assetCode, assetType: 7, subtype: '72' }, this.$message)
