...
import {
  ...
  downloadList,
  unpackMainReferAsset,
  getTree
} from '@/api/index';
import { checkAssetCode, checkAssetName, checkVersion, checkStoreName } from '@/utils/validate'
import { DefaultValue, AssetCodeToolTipText, assetTypeListDailog } from '@/config/index'
// import { getToken } from '@/utils/auth'
import { getAssetTypeLabel, getCodePrefix, getRandomCode } from '@/utils'
export default {
  name: 'SelectDownload',
  ...
  data() {
    return {
      assetTypeList: assetTypeListDailog,
  ...
  methods: {
    handleChange(arr) {
      this.requestData.assetType = arr[0]
      this.requestData.subtype = arr[1]
    },
    // 获取专题树
    getTreeList() {
      getTree().then(rs => {
    ...
    getDownloadAssList() {
      downloadList({
        packName: this.requestData.assetName,
        ...
        this.showList = res.data.list.map(item => {
          ...
          item.id = item.shopCode + item.packVersion
    ...
    next() {
      ...
      if (!this.selectTableList[0].packFilePath) {
        return this.$message.warning('当前资源没有资源包，请重新选择')
      }
      this.formData.assetCode = getRandomCode(this.selectTableList[0].subtype)
      this.formData.assetCodePrefix = getCodePrefix(this.selectTableList[0].subtype)

      this.formData.packFilePath = this.selectTableList[0].packFilePath
      this.formData.assetName = this.selectTableList[0].assetName
      this.formData.assetType = this.selectTableList[0].assetType
      this.formData.subtype = this.selectTableList[0].subtype
      console.log(this.selectTableList[0], '------选择的数据')
      unpackMainReferAsset({ packFilePath: this.formData.packFilePath }).then(res => {
        this.subAssNameList = res.data && res.data.referAssetList && res.data.referAssetList.map(item => item.assetName) || []
        this.subAssNameList2 = res.data && res.data.referSubAssetList && res.data.referSubAssetList.map(item => item.assetName) || []
        this.step = 2
      })
    },
    pre() {
        ...
        if (valid) {
          if (this.subAssNameList.length || this.subAssNameList2.length) {
            ...
            if (this.subAssNameList2.length) {
              str = `<strong class="confirm-title">请确认是否新建该资源？</strong><br><p class="confirm-subtitle">${this.subAssNameList.length ? '该资源引用的<span class="num">' + this.subAssNameList.length + '</span>个资源（' + this.subAssNameList.join('、') + '）、' : ''}订阅的<span class="num">${this.subAssNameList2.length}</span>个资源（${this.subAssNameList2.join(',')}）将同步创建，使用订阅资源需在资源商城中完成资源订阅，获取授权信息`
            } else {
              str = '<strong class="confirm-title">请确认是否新建该资源？</strong><br><p class="confirm-subtitle">该资源引用的<span class="num">' + this.subAssNameList.length + '</span>个资源（' + this.subAssNameList.join('、') + '）将同步创建</p>'
            }
            this.$confirm(
              str,
              '提示',
