...
<template>
  <c-dialog
    ...
    <div class="box-wrap">
      <h3 class="info-title" style="margin-bottom:16px">基本信息</h3>
      <c-row>
        <c-col :span="12">
        ...
        <c-col :span="24">
          <div class="cls-item" data-label="接口说明"><span v-html="dataInfo.desc"></span></div>
        </c-col>
      </c-row>
  ...
  methods: {
    async getApiData() {
      try {
        ...
        const res = await queryApimInfo({ bizCode: this.bizCode, packVersionId: this.packVersionId })
        const data = res.data
