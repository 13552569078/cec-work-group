...
import { tagTree, assetCodeExist, assetNameExist, apiAssetListPage, techInterfaceDetail, techInterfaceAdd, techInterfaceUpdate, assetReferenced } from '@/api/index'
...
export default {
  ...
  methods: {
    ...
    initData() {
      // 获取详情
      techInterfaceDetail(this.$route.query.id).then(res => {
    ...
    handleOperate(item) {
      if (item.label === '更多') {
        this.$router.go(-1)
        openAssetEditorUrl({ ...this.formData, assetType: 2, subtype: '22', readonly: this.pageStatus === 'detail' ? 1 : '' }, this.$message)
    ...
    saveForm() {
      this.$refs.dataForm.validate((valid) => {
        ...
        if (this.pageStatus === 'add') {
          techInterfaceAdd({ ...this.formData, referAssetList, referPrivateAssetList, referShareAssetList, assetCode: (this.formData.assetCodePrefix || '') + this.formData.assetCode }).then(res => {
            if (res.code * 1 === 0) {
              this.$message.success(res.msg)
              this.$router.go(-1)
              openAssetEditorUrl({ ...this.formData, assetCode: (this.formData.assetCodePrefix || '') + this.formData.assetCode, assetType: 2, subtype: '22' }, this.$message)
            }
          })
        } else {
          techInterfaceUpdate({ ...this.formData, referAssetList, referPrivateAssetList, referShareAssetList, assetCode: (this.formData.assetCodePrefix || '') + this.formData.assetCode }).then(res => {
            if (res.code * 1 === 0) {
              this.$message.success(res.msg)
              this.$router.go(-1)
              openAssetEditorUrl({ ...this.formData, assetCode: (this.formData.assetCodePrefix || '') + this.formData.assetCode, assetType: 2, subtype: '22' }, this.$message)
