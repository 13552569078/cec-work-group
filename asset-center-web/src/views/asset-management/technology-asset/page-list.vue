      ...
      >
        <template v-slot:header="{searchForm}">
          ...
          >
            ...
            <c-form-item label="页面类型" class="lp-search-form-field">
              <c-select v-model="searchForm.businessType" :specific-suffix="true" class="form-input-item" size="small"
                        placeholder="请选择">
        ...
        <template v-slot:content-header_right>
          <button-dropdown v-show="getPermissionByAssetsbutype(23, 'copyAsset')" class="btn-operate mrg-right" :btn-type="'operate'" :options="btnList" :text="'复制资源'" @command="handleCommand" />
          <c-button
            v-show="getPermissionByAssetsbutype(23, 'add')"
          ...
          <div v-show="pageListType=='card'" class="assets-content-list" :style="{'height': setting.tableHeight,}">
            <div class="list" :style="{ width: '1619px' }">
              <assetsCard v-for="asset in dataList" :key="asset.assetId" class="assets-item" :is-list="true" :show-status="true"
                          :page-key="'pageAsset'" :data="asset" :btn-list="getbtns(asset)" @handleClick="handleOperateClick"></assetsCard>
            </div>
          </div>
          <assetsTable
...
import { assetSrc, assetStatus, shareStatus, pageTypeList } from '@/config/index'
    ...
    handleCommand(command) {
      ...
      switch (+command) {
        case 1:// 新增
          ...
          break;
        case 3:// 公共
          this.showCommonDialog({ assetType: '2', subtype: '23' })
          break;
        case 4:// 下载
          this.showSelectAssetDialog({ assetType: '2', subtype: 23 })
          break;
        case 5: // 导入
          this.showImportDialog({ assetType: '2', subtype: '23' })
          break;
