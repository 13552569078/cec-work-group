...
import tiList from './ti-list.vue'
...
export default {
  ...
  data() {
    let tabList = [
      { label: '体征', value: 'tiAsset', hide: !getUserPermissionByKey('tiAsset') }
    ]
    tabList = tabList.filter(d => !d.hide)
    let activeTab = window.sessionStorage.getItem('StatisticsAssetactiveTab')
  ...
  methods: {
    cache() {
      window.sessionStorage.setItem('StatisticsAssetactiveTab', this.activeTab)
    },
    clearQueryCache() {
      window.sessionStorage.setItem('StatisticsAssetactiveTab', '')
    }
  }
