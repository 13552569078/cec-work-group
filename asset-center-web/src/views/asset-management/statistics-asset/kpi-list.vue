  ...
  <div class="kpi-main">
    ...
    >
      ...
      <template v-slot:content-header_right>
        <!-- <c-button type="ordinary" size="small" class="btn-operate btn-import mrg-right" @click="showImportDialog({ assetType: '1', subtype: '11' })">导入资源</c-button> -->
        <button-dropdown v-show="getPermissionByAssetsbutype(11, 'add')" :options="btnList" :text="'新建资源'" @command="handleCommand" />
      </template>
        ...
        <div class="assets-content-list" :style="{'height': setting.tableHeight,}">
          <div class="list" :style="{ width: '1619px' }">
            <assetsCard v-for="asset in dataList"
                        :key="asset.assetId" :page-key="'kpiAsset'" class="assets-item" :is-list="true" :show-status="true" :data="asset" :btn-list="getbtns(asset)" @handleClick="handleOperateClick"></assetsCard>
          </div>
        </div>
      </template>
    ...
    <c-dialog :class="'dialog-center'" :title="title" :visible.sync="kpiDialogFlag" width="666px" :before-close="closeDialog">
      <kpi-add
        v-if="kpiDialogFlag"
        :flag="flag"
        :tag-tree-data="tagTreeDataOld"
    ...
    <selectDownload v-if="dialogVisibleSelect" :group-id="groupId" :dialog-flag="dialogVisibleSelect" :asset-type="currAsset.assetType" :subtype="currAsset.subtype" @close="closeSelectDialog" />
  </div>
</template>
...
import kpiAdd from './components/kpi-add.vue'
...
export default {
  ...
  data() {
    return {
      title: '',
      btnList: [{ label: '新建空白资源', code: '11' }, { label: '选择下载资源' }],
      defaultExpandedKey: [],
      assetSrc: assetSrc,
      assetStatus,
  ...
  methods: {
    ...
    handleCommand(command) {
      ...
      if (!command) {
        return this.showSelectAssetDialog({ assetType: '1', subtype: '11' })
      }
    ...
    handleOperateClick(msg) {
      switch (msg.label) {
        ...
        case '查看':
          ...
          break
        case '指标设计':
          this.handleDetail(msg.row, 'desing')
          break
        case '编辑':
    ...
    closeDialog() {
      this.kpiDialogFlag = false
      this.currAssetId = null
      this.getList()
    },
    ...
    handleDetail(row, flag = 'detail') {
      ...
      if (flag === 'desing') {
        // 跳转webview
        return this.gotoWebview(row)
      }
      this.$router.push({
        path: '/asset-management/statistics-asset/kpi-asset/add',
        query: Object.assign({}, {
          id: row.id,
          flag: flag,
          cache: true
    ...
    async getList() {
      ...
      try {
        ...
        queryParams.sceneId = this.sceneId || 0
        const result = await getIndexTableList(queryParams)
        this.dataList = result.data.list.map(item => {
          if (item.status === 1)item.version = ''
          return item
        })
        this.page.total = result.data.total
        this.doQueryCache(['searchForm', 'page', 'sceneId', 'activeTab', 'tags'])
      } catch (err) {
        console.log(err)
      }
      this.loading = false
