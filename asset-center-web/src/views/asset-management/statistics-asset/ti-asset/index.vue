  ...
  methods: {
    ...
    saveForm() {
      this.$refs.dataForm.validate((valid) => {
        if (!valid) return;
        if (this.pageStatus === 'add') {
          citysignsAdd({ ...this.formData, assetCode: (this.formData.assetCodePrefix || '') + this.formData.assetCode }).then(res => {
            if (res.code * 1 === 0) {
          ...
          citysignsUpdate({ ...this.formData, assetCode: (this.formData.assetCodePrefix || '') + this.formData.assetCode }).then(res => {
            if (res.code * 1 === 0) {
              this.$message.success(res.msg)
              this.$router.go(-1)
              openAssetEditorUrl({ ...this.formData, assetCode: (this.formData.assetCodePrefix || '') + this.formData.assetCode, assetType: 1, subtype: '14' }, this.$message)
