<template>
  ...
  >
    ...
    >
      ...
      <c-form-item
        v-if="currAsset.subtype+''==='51' ||currAsset.subtype+''==='13' "
        label="场景分类"
        prop="groupId"
        class="half-form-item"
      >
...
export default {
  ...
  created() {
    ...
    this.getTreeList()
    this.$nextTick(() => {
      this.formData.assetCode = getRandomCode(this.currAsset.subtype)
      this.formData.assetCodePrefix = getCodePrefix(this.currAsset.subtype)
    })
  },
  mounted() {
  },
  methods: {
    ...
    handleAvatarSuccess(res, file) {
      res = res.data || {}
      if (!this.currAsset ||
        !this.currAsset.assetType ||
        this.currAsset.assetType + '' !== res.assetType + '' ||
        this.currAsset.subtype && this.currAsset.subtype + '' !== res.subtype + ''
      ) {
        this.$refs['upload-btn'].clearFiles()
        return this.$message.error('资源类型不符，请重新上传资源包！')
      }
      this.parseRes = res
