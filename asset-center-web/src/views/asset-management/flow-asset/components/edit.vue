...
<template>
  <div style="width:100%;height:100%">
    <detail-layout :page-title="'基本信息'"
      ...
      <template v-slot:content>
        ...
        <div v-show="pageStatus!='detail'" ref="base-info" class="white-box-wrap">
          ...
          <c-form ref="dataForm" :model="formData" label-width="108px" class="cls-model-form"
              ...
              <c-row :gutter="0" class="search-box">
                ...
                <c-col :span="span">
                  ...
                  <c-form-item label="" prop="assetCode">
                    ...
                    <c-input v-model="formData.assetCode"
                             :placeholder="pageStatus === 'detail'?'':'请输入资源编码'" size="small"
                             clearable class="form-input-item "
                      ...
                      <template v-if="pageStatus === 'add'"
                                slot="prepend">{{ formData.assetCodePrefix }}</template>
                    </c-input>
                  </c-form-item>
                ...
                <c-col :span="span">
                  <c-form-item label="场景分类" prop="groupId">

                    <c-cascader v-model="formData.groupId" style="width:100%" size="small"
                                :specific-suffix="true" :show-all-levels="false" :options="treeData"
                                class="form-input-item" :props="{
                                  multiple: false,
                                  value: 'groupId',
                                  label: 'groupName',
                                  children: 'childList',
                                  emitPath: false,
                                  checkStrictly: true
                                }" :placeholder="pageStatus === 'detail' ? '' : '请选择'" clearable />
                  </c-form-item>
                </c-col>
        ...
        <c-form ref="dataForm2" :model="formData" label-width="108px" class="cls-model-form"
                :class="pageStatus === 'detail'?'disabled-new':''"
            ...
            <c-row :gutter="0" class="search-box">
              <c-col :span="span *2">
                <c-form-item label="" :disabled="false">
                  <publishAsset :provide="false" :show-list="formData.referKpiList"
                                :page-status="pageStatus" @goDetail="goDetail"
                                @deleteRefer="deleteRefer" @add="openIndexTableDialog"></publishAsset>
                </c-form-item>
              </c-col>
            </c-row>
          </div>
          ...
          <div v-if="pageStatus === 'detail'&& formData.status===5" class="white-box-wrap">
            ...
            <c-row :gutter="0"
                   class="search-box">
              <c-col :span="span *2">
                <c-form-item label="" style=" margin-top: 20px; ">
    ...
    <referrassetDialog v-if="indexDialogVisible" title="引用发布资源" checked-name="assetId"
    ...
    <expandTable v-if="indexDialogVisible2" title="引用订阅资源" checked-name="assetId"
...
import { tagTree, getTree, getDetail, assetCodeExist, assetNameExist, apiAssetListPage, processUpdate, processAdd, getByBizCode, assetReferenced, getByBasicInfo } from '@/api/index'
...
export default {
  ...
  methods: {
      ...
      this.$confirm(
        ...
        .then((res) => {
          if (subIndex !== undefined) {
            ...
            console.log(index, subIndex, this.formData.referSubAssetList[index].referApiList, 'index')
            ...
            console.log(this.formData.referSubAssetList[index].referApiList.length, '---------------')
    ...
    getDialogTableList(params) {
      const realParams = Object.assign({ createSrc: 1 }, params, this.pageStatus === 'add' ? {} : { assetId: this.$route.query.id * 1, filterAssetCode: this.formData.assetCode })
    ...
    getDialogTableList2(params) {
      const realParams = Object.assign({ createSrc: 2 }, params, this.pageStatus === 'add' ? {} : { assetId: this.$route.query.id * 1, filterAssetCode: this.formData.assetCode })
      apiAssetListPage(realParams).then(res => {
