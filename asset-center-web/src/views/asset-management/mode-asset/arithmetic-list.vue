    ...
    <div class="tech-wrap">
      ...
      >
        ...
        <template v-slot:content-header_right>
          <c-button
            v-show="getPermissionByAssetsbutype(71, 'import')"
            type="ordinary" size="small" class="btn-operate btn-import mrg-right" @click="showImportDialog({ assetType: '7', subtype: '71' })">导入资源</c-button>
          <button-dropdown
            v-show="getPermissionByAssetsbutype(71, 'add')"
...
export default {
  ...
  data() {
    return {
      btnList: [{ label: '新建空白资源', code: '71' }, { label: '选择下载资源' }],
  ...
  methods: {
    ...
    handleDetail(row, flag = 'detail') {
      if (row.subtype + '' === '71') { // 模型
        this.$router.push({
          path: '/asset-management/mode-asset/arithmetic-asset',
          query: Object.assign({}, {
            id: row.id,
            cache: true,
            flag
    ...
    handleCommand(command) {
      ...
      if (!command) {
        return this.showSelectAssetDialog({ assetType: '7', subtype: '71' })
      }
    ...
    async getList() {
      ...
      try {
        ...
        queryParams.subtype = '71' //
        queryParams.assetType = '7' //
        const result = await modeListPage(queryParams)
