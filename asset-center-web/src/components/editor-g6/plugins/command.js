      ...
      method: function (graph) {
        ...
        if (selectedItems && selectedItems.length > 0) {
          selectedItems.forEach(i => {
            ...
            const deleteNode = node.get('model');
            let edge;
            if (deleteNode.clazz === 'flow') {
              edge = deleteNode;
            } else {
              edge = node.getOutEdges()[0];
              edge = edge ? edge.get('model') : undefined;
            }
            if (edge) {
              // 删除源或者线时清空target的operator数据
              const targetNode = graph.findById(edge.target).get('model');
              targetNode.data.output = [];
              targetNode.data.operator = undefined;
              graph.updateItem(edge.target, targetNode);
            }
            if (node) {
              graph.remove(i);
            } else {
            ...
            if (deleteNode && deleteNode.clazz !== 'flow') {
              // 组件名称不能重复功能
              const nodeArr = graph.getNodes() || [];
              const currentModelNames = {
                input: [],
                node: []
              };
              nodeArr.forEach(node => {
                const desc = node.get('model');
                if (desc && desc.clazz === 'input') {
                  currentModelNames.input.push(desc.label);
                } else if (desc && desc.clazz === 'end') {
                  currentModelNames.node.push(desc.label);
                } else if (desc && desc.data && desc.data.name) {
                  currentModelNames.node.push(desc.data.name);
                }
              })
              store.dispatch('settings/changeCurrentModelNames', currentModelNames)
            }
          });
        }
