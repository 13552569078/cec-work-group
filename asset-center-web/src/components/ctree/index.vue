...
<template>
  <div class="ct-main">
    ...
    <div class="bg-fff tree-box" :style="' height: '+treeHeight+';'">
      <div v-show="hasroot && !isview&&operatable" class="tree-plus">
        <em class="iconfont c-icon-circle-plus-outline"
            @click="addTreeNode({})"
        ></em>
      </div>
      <c-tree
      ...
      >
        <div
          ...
          :class="{isroot: !data.id}"
        >
          <node-label :label="node.label">
          </node-label>
          <div v-show="!isview&&!data.readOnly&&operatable" class="right-node">
...
export default {
  ...
  methods: {
    deleteTree(node) {
      ...
      //   iconClass: 'confirm',
      //   customClass: 'message-confirm-wrap'
      // }).then(() => {
...
.ct-main .ct-tree {
  ...
  .c-tree-node__content:hover {
    background: #F9FAFE;
  }
  &.c-tree--highlight-current .c-tree-node.is-current > .c-tree-node__content {
    background: #F9FAFE;
  }
}
.details-dialog-adjustpadding-1 .c-form-item__label:before {
  display: none !important;
}
.group-form-item {
  margin-bottom: 0;
}
.details-dialog-adjustpadding-1 .c-dialog {
  position: relative;
  top: -98px;
}
</style>
