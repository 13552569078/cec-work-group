      ...
      <div class="body-demo">
        ...
        >
          ...
          <template v-for="(item,index) in tableCloumnData">
            ...
            <c-table-column
              v-else
              :key="`checkedNadme-${item.checkedName}-${index}`"
              :label="`${item.lable}`"
              :prop="`${item.prop}`"
              width="280"
            >
  ...
  methods: {
    ...
    getKpiList(needClear) {
      if (needClear) {
        ...
        this.requestData.appName = ''
        this.requestData.appCode = ''
      ...
      if (this.selectTableList.length) {
        ...
        for (let index = 0; index < this.selectTableList.length; index++) {
          ...
          if (!(this.selectTableList.findIndex(_item => {
            return item.appCode === _item.appCode
          }) === index)) {
            uniqueFlag = false
            break
          }
    ...
    userClickSelect(arrs, item) {
      ...
      if (item) {
        ...
        if (this.selectTotalList.indexOf(item[`${this.checkedName}`]) > -1) {
          ...
          // 选中
          const repeatAssets = this.selectTableList.filter(_item => _item.appCode === item.appCode) // 相同资源的不同版本数组
          if (repeatAssets.length) { // 有相同资源的版本
            this.$message.warning('已更改选择，同个资源不可选择多版本')
            this.selectTableList = this.selectTableList.filter((i) => i.appCode !== item.appCode)
