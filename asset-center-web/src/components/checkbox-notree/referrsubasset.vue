...
<template>
  <c-dialog
    ...
    @close="dialogClose"
  >
    <template v-if="provide" slot="title">
      <span style="display: flex;align-items: center;line-height: 20px;font-size: 14px; color: #4B4B4B; font-weight: 500;">
        ...
        <c-tooltip placement="right" popper-class="cls-form-label-tip">
          <span slot="content" class="hahah" v-html="'仅能引用通过当前应用的关联应用订阅的资源'">
          </span>
...
export default {
  ...
  data() {
    return {
      ...
      treeData: [],
      localShowList: [].concat(this.showList),
      dialogVisible: true,
...
assetCodePrefix: '',
  ...
  methods: {
    ...
    renderTotalList(isFirst = false) {
      getApiList(this.showList.map(item => item[this.checkedName])).then(res => {
