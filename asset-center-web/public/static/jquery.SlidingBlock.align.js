...
(function ($) {
  // 定义插件名称
  var pluginName = "SlidingBlockAlign";
  ...
  var defaults = {
    ...
    slider_w: '4px', //滑块宽度   需要写单位
    slider_left: 0, //滑块right 定位 需要写单位
    slider_class: 'sliding-block-align', //滑块的通用class
  ...
  Plugin.prototype = { //全局变量
    init: function () { // 初始化
      ...
      _this.mouseFollow(_this, _slider_move);

    },
    mouseFollow: function (_this, _slider_move) {
      var _ele = $(_this.elements); // ul
      var _timer = null;
      _ele.stop(true, true).on(_this.settings.event_type, _this.settings.ele_child, function (e) {
        var _li = $(this),
          _liHeight = _li.outerHeight(),
          _liPosTop = _li.position().top
        if (_timer) {
          clearTimeout(_timer);
        }
