<template>
  <div v-if="!item.hidden" class="c-menu-item-router">
    <template
      ...
      <app-link v-if="onlyOneChild.meta&&!onlyOneChild.meta.openA" :to="resolvePath(onlyOneChild.path)">
        <c-menu-item
      ...
      <c-menu-item
        v-if="onlyOneChild.meta&&onlyOneChild.meta.openA" :index="resolvePath(onlyOneChild.path)" :class="{'submenu-title-noDropdown':!isNest}"
        :style="collapseStyle"
        @click="openA(onlyOneChild.meta.type)"
      >
        <item :icon="onlyOneChild.meta.icon||(item.meta&&item.meta.icon)"
              :title="onlyOneChild.meta.title" />
      </c-menu-item>
...
export default {
  ...
  methods: {
    ...
    openA(routePath) {
      ...
      if (routePath === 'shopWeb') {
        return window.open(window.__systemConfig.assetShopUrl)
      }
    },
