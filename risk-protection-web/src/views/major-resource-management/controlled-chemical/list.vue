...
const tableData = ref([] as IpostFindControlledChemicalsPageSpace.List[]);
const tableRef = ref();
...
const getData = async({ pageSize, currentPage, params, setTotal }: { pageSize: number; currentPage: number; params: { [key: string]: string | string[] }; setTotal: (item?: number) => void }) => {
  ...
  const { data } = await apiControlledChemicalsManagementObj.postFindControlledChemicalsPage(requestParams);
...
const handleDel = async(id: string, name: string) => {
  ...
  await apiControlledChemicalsManagementObj.postDeleteControlledChemicals(id);
