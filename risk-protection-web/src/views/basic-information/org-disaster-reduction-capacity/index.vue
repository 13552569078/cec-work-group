<template>
  ...
  >
    ...
    <template #toolbar-extra-after>
      <fx-action-button label="新增社会组织减灾能力" class="m-x-md" @click="edit('add')" />
...
import type { IpostFindDisasterSocietyReductionPageSpace } from '@/apis/modules/basic-information/model';
import type { ActionType } from '@/types';

const CODE = 'DisasterSocietyReduction';

const { getAreaTree, areaProps, areaTreeData } = useAreaTree();

const tableRef = ref();
const tableData = ref([] as IpostFindDisasterSocietyReductionPageSpace.List[]);
...
const getData = async({
  ...
  const { data } = await apiBasicInformationObj.postFindDisasterSocietyReductionPage(requestParams);
...
const del = async(id: string, name: string) => {
  ...
  await apiBasicInformationObj.postDeleteDisasterSocietyReduction([id]);
  HlMessage.success("删除成功");
  tableRef.value.refresh();
};
const batchDel = async(ids: string[]) => {
  await HlMessageBox.confirm(`此操作将批量删除这些社会组织减灾能力, 是否继续?`, "提示", {
    confirmButtonText: "确定",
    confirmButtonType: "danger",
    cancelButtonText: "取消",
  });
  await apiBasicInformationObj.postDeleteDisasterSocietyReduction(ids);
