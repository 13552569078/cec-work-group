  ...
  >
    ...
    <template #actions="{ row }">
      <hl-group align="items-between">
        ...
        <hl-button type="link" effect="light" @click="del(row.id, row.areaCodeName)"> 删除 </hl-button>
      </hl-group>
    </template>
    <template #toolbar-extra-after>
      <fx-action-button label="新增乡镇减灾能力" class="m-x-md" @click="edit('add')" />
...
import type { IpostFindDisasterTownshipReductionPageSpace } from '@/apis/modules/basic-information/model';

const CODE = 'DisasterTownshipReduction';

const { getAreaTree, areaProps, areaTreeData } = useAreaTree();

const tableRef = ref();
const tableData = ref([] as IpostFindDisasterTownshipReductionPageSpace.List[]);
...
const getData = async ({
  ...
  const { data } = await apiBasicInformationObj.postFindDisasterTownshipReductionPage(requestParams);
  data.list.forEach(el => {
     el.materialsReserveMethodName = (el.materialsReserveMethod.split(',').map(el => DICT.RESERVE_METHOD.filter(d => +d.code === +el)[0]?.name)).join('、') || '--';
     el.disasterReportMethodName = (el.disasterReportMethod.split(',').map(el => DICT.REPORTING_METHO.filter(d => +d.code === +el)[0]?.name)).join('、')  || '--';
     el.disasterReceiveMethodName = (el.disasterReceiveMethod.split(',').map(el => DICT.REPORTING_METHO.filter(d => +d.code === +el)[0]?.name)).join('、')  || '--';
     el.disasterTypeName = (el.disasterType.split(',').map(el => DICT.DISASTER_TYPE.filter(d => +d.code === +el)[0]?.name)).join('、') || '--';
  })
...
const del = async (id: string, name:string) => {
  ...
  await apiBasicInformationObj.postDeleteDisasterTownshipReduction([id]);
  HlMessage.success("删除成功");
  tableRef.value.refresh();
};
const batchDel = async (ids: string[]) => {
  await HlMessageBox.confirm(`此操作将批量删除这些乡镇, 是否继续?`, "提示", {
    confirmButtonText: "确定",
    confirmButtonType: "danger",
    cancelButtonText: "取消",
  });
  await apiBasicInformationObj.postDeleteDisasterTownshipReduction(ids);
