<template>
  ...
  >
    ...
    <template #toolbar-extra-after>
      <fx-action-button label="新增政府灾害管理能力" class="m-x-md" @click="edit('add')" />
...
import type { IpostFindDisasterGovernmentManagementPageSpace } from '@/apis/modules/basic-information/model';
import type { ActionType } from '@/types';

const CODE = 'DisasterGovernmentManagement';

const { getAreaTree, areaProps, areaTreeData } = useAreaTree();

const tableRef = ref();
const tableData = ref([] as IpostFindDisasterGovernmentManagementPageSpace.List[]);
...
const cols = ref([
  ...
  },
  {
    ...
    minWidth: "120px",
    showTooltip: true,
    formatter: (_: any, __: any, cellValue: number) => DICT.AGENCY_LEVEL.find(a => a.code === cellValue)?.name || '--',
    tooltipProps: { maxWidth: "400px" },
  },
  {
    title: "灾害管理领域",
    prop: "disasterManagementField",
    showTooltip: true,
    formatter: (_: any, __: any, cellValue: string) => cellValue.split(',').map(c => DICT.DISASTER_MGMT_DOMAIN.find(a => a.code === c)?.name || '--').filter(Boolean).join(','),
    tooltipProps: { maxWidth: "400px" },
  },
  {
    title: "直属涉灾事业单位数",
    prop: "publicInstitutionsNumber",
    minWidth: "80px",
    showTooltip: true,
    tooltipProps: { width: "400px" },
  },
  {
    title: "灾害管理人员总数",
    prop: "disasterManagementPersonnelTotal",
    minWidth: "100px",
    showTooltip: true,
    tooltipProps: { width: "400px" },
  },
...
const getData = async({
  ...
  const { data } = await apiBasicInformationObj.postFindDisasterGovernmentManagementPage(requestParams);
...
const del = async(id: string, name: string) => {
  ...
  await apiBasicInformationObj.postDeleteDisasterGovernmentManagement([id]);
  HlMessage.success("删除成功");
  tableRef.value.refresh();
};
const batchDel = async(ids: string[]) => {
  await HlMessageBox.confirm(`此操作将批量删除这些政府灾害管理能力, 是否继续?`, "提示", {
    confirmButtonText: "确定",
    confirmButtonType: "danger",
    cancelButtonText: "取消",
  });
  await apiBasicInformationObj.postDeleteDisasterGovernmentManagement(ids);
