<template>
  ...
  >
    ...
    <template #toolbar-extra-after>
      <fx-action-button label="新增化工园区" class="m-x-md" @click="edit('add')" />
...
import type { IpostFindDisasterChemicalIndustrialParkPageSpace } from '@/apis/modules/basic-information/model';
import type { ActionType } from '@/types';

const CODE = 'DisasterChemicalIndustrialPark';

const { getAreaTree, areaProps, areaTreeData } = useAreaTree();

const tableRef = ref();
const tableData = ref([] as IpostFindDisasterChemicalIndustrialParkPageSpace.List[]);
...
const getData = async({
  ...
  const { data } = await apiBasicInformationObj.postFindDisasterChemicalIndustrialParkPage(requestParams);
...
const del = async(id: string, name: string) => {
  ...
  await apiBasicInformationObj.postDeleteDisasterChemicalIndustrialPark([id]);
  HlMessage.success("删除成功");
  tableRef.value.refresh();
};
const batchDel = async(ids: string[]) => {
  await HlMessageBox.confirm(`此操作将批量删除这些化工园区, 是否继续?`, "提示", {
    confirmButtonText: "确定",
    confirmButtonType: "danger",
    cancelButtonText: "取消",
  });
  await apiBasicInformationObj.postDeleteDisasterChemicalIndustrialPark(ids);
