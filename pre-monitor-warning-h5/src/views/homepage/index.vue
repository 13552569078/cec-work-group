<template>
  <van-nav-bar title="监测预警" style="--van-nav-bar-icon-color: var(--van-text-regular)" />
  <div class="grid-wrap">
    <div class="item" v-for="(i, index) in list" :key="index" @click="handleGridClick(i)">
      <img class="icon" :src="i.icon" />
      <div class="title">{{ i.title }}</div>
    </div>
  </div>
</template>

<script setup lang="ts">
import report from "@/assets/images/homepage/report.png";
import task from "@/assets/images/homepage/task.png";
import { useUser } from "fx-front-ucenter";
import { useRouter } from "vue-router";
import { useCookie } from "@/hooks";
const { getToken, setToken } = useUser();
const { getCookie } = useCookie();

interface menu {
  icon: any;
  title: string;
  routerName: string;
}

// 磁县环境，cookie获取fxtoken
const fxtoken = getCookie("fxtoken") as string;
// console.log(fxtoken, 'fxtoken1');
fxtoken && setToken(fxtoken);
// test data
// setToken('eyJhbGciOiJSUzI1NiJ9..gK84OIffuI5muHHemlFaWzLzglCvNFrDwfRycUAufYuJrRGiD0ythZhzgv7Pz2A0Guy2yWC7k6MBZsocoCLPYB0PzrJCmafxfJdxjbnkG9OxPVuyRBYb_o-jZS2DRZwWGAS9rmbOHpwNWjzqOBbZVUMugNNdWPkTOatngtvuxay9tJu_fooXAF7kK14x_jRq3-mZdqNESAVzfd0nXGtqS0x1rIKtHSIsb_ILf4qWTtRhsFdic8ABXZd7hhQZ3S_hAjQymoyj9ESJ8KpeaZ8ev02VIyc9dTBFgMWYq6GCL-oL20YlaoRfREJUKiigFCe3-CVVGtuNUrsJEhFXmQku3A');
// console.log('homepageIndex1', getToken());
// console.log(report, 33);
const router = useRouter();

const list = [
  {
    icon: report,
    title: "实时预警",
    routerName: "RealtimeWarningList",
  },
  {
    icon: task,
    title: "历史预警",
    routerName: "HisyoryWarningList",
  },
];
const handleGridClick = (item: menu) => {
  router.push({ name: item.routerName });
};
// app测试写死token、演示的时候url获取token
const handleAppTest = () => {
  const searchParams = new URLSearchParams(window.location.search); // H5模式
  const token = searchParams.get("token");
  if (token) {
    localStorage.setItem("$UCENTER_TOKEN", token);
  } else {
    document.addEventListener("UniAppJSBridgeReady", () => {
      uni.getEnv((res: { plus: any }) => {
        // console.log('当前环境：' + JSON.stringify(res));
        if (res?.plus && !getToken()) {
          localStorage.setItem(
            "$UCENTER_TOKEN",
            "eyJhbGciOiJSUzI1NiJ9..vrkSH-Ye3oTqp4K6V_8OmxKBRVAbUWoAjXhGr6b_dLEMo8UmVLPHA6KcysrtZrP7p0pMA1rPNcCrg2yWbP3hYZqGsqiYd4AIRZfFINOu-CqVeAKA-r2Z8jZhqvgCPvREdXbV8VZZAUgJJWIPRQIELLissL97nUVIrbIOX3TsmZr5VBmwEUEnKwF1jZBXDU1XbPndr7tfn6_fg8I5R4gC3Zk-jeZbQW5IIDBDXF5rW3pX7sr2f5FQRoxglKavzFPzfbAHDe3UeT-wXZlk6VQICAC2peo3IQ6oxBhsIcwVEYXs-xg9U2vqBiSOOmXJKmlRrpvhksOzVtI-ikFdagtWgg"
          );
        }
      });
    });
  }
};
handleAppTest();
</script>
<style lang="scss" scoped>
.grid-wrap {
  border-radius: 8px;
  display: flex;
  flex-direction: row;
  flex-wrap: wrap;
  padding: 20px;
  margin: 16px;
  gap: 24px;
  background: #ffffff;

  .item {
    .icon {
      width: 48px;
      height: 48px;
    }

    .title {
      font-size: 12px;
      line-height: 20px;
    }
  }
}
</style>
