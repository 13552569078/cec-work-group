<template>
  <hl-upload
    ...
    :class="['task-file-form-item', curFileList.length >= FILE_COUNT_LIMIT ? 'hide-upload' : '']"
  >
    ...
    <template #file-preview="{ file }">
      <task-file-card :item="setFileCardItem(file)" style="position:absolute;" />
    </template>
  </hl-upload>
</template>
...
import { TaskAttachmentParams } from '@/types';
...
const props = defineProps({
  attachmentList: {
    type: Array as PropType<TaskAttachmentParams[]>,
    default: () => ([])
  },
});
const emit = defineEmits(['change']);

const curFileList = ref<TaskAttachmentParams[]>([]);

const setFileCardItem = (f: any) => {
  const name = f.response ? f.response?.data.originalName : (f.originalName || f.name || f.taskFileName);
  const res = {
    id: f.id || f.response?.data.id,
    name,
    url: f.response ? '/' + f.response?.data.dirUrl : (f.url || f.taskAttachmentUrl),
...
const handleAttachments = (fileList: Record<string, any>[]) => {
  curFileList.value = fileList.map(f => ({
    taskInfoId: '',
    id: f.id || f.response?.data.id,
    taskAttachmentName: f.response ? f.response?.data.originalName : (f.originalName || f.name || f.taskFileName),
    taskFileName: f.response ? f.response?.data.originalName : (f.originalName || f.name || f.taskFileName),
    taskAttachmentUrl: f.response ? '/' + f.response?.data.dirUrl : (f.url || f.taskAttachmentUrl),
    createdAt: f.createdAt
  }));
  console.log('handleAttachments....', curFileList.value);
  emit('change', curFileList.value);
};
...
const previewMediaFile = (file: any) => {
  const name = file.response ? file.response?.data.originalName : (file.originalName || file.name || file.taskFileName);
  const url = file.response ? (file.response as any)?.data.url : (file.url || file.taskAttachmentUrl);
