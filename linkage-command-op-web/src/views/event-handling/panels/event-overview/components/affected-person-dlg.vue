<template>
  ...
  >
    ...
    >
      <dynamic-form-item v-for="idx in personIdxList" :key="idx.code" :idx="idx"/>
    </hl-form>
    <template #footer>
...
import { nonNegativeIntValidator, PERSON_INJURY_CARD } from '@/common';
...
const personIdxList = computed(() => props.idxList.filter(i => i.displayCard === PERSON_INJURY_CARD.value));

const initFormData = (currentEvent: IpostFindEventDetailByIdSpace.Data) => {
  currentEvent.indicies.filter(i => i.displayCard === PERSON_INJURY_CARD.value).forEach(i => {
