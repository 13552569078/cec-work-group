<template>
  <div class="risk-table-part">
    <div class="title">周边风险</div>
...
const allRiskTypeList = computed(() => {
  return ALL_RESOURCES_TREE_NEW.value?.find(el => el.id === RISK_NAME)?.children.map(i => { return { label: i.label, value: i.id }; });
});
const intelligenceResourceTypeList = computed(() => {
  return allConfigResourceType.value?.find(el => el.id === RISK_NAME)?.children;
});
...
const searchResource = async() => {
  ...
  const res = await apiDisposeSuggestObj.postFindEventSurroundingItems(params);
