<template>
  <div class="department-comment-part">
  ...
  <preview-file-dlg v-if="showPreviewDlg" @cancel="showPreviewDlg = false" :file-url="curFileUrl" />
</template>
...
import type { IpostFindDepartmentAdvicePageSpace } from '@/apis/modules/dispose-suggest/model';
...
const tableData = ref<IpostFindDepartmentAdvicePageSpace.List[]>([]);
const cols = [
  { title: '',
...
const handlePreview = (row: IpostFindDepartmentAdvicePageSpace.List) => {
  const { adviceFile } = row;
  curFileUrl.value = adviceFile;
  showPreviewDlg.value = true;
};
const searchDepartmentComments = async() => {
  const params = {
    currentPage: -1,
    pageSize: 9999,
    eventId: currentEvent.id as string
  };
  const res1 = await apiDisposeSuggestObj.postFindDepartmentAdvicePage(params);
...
onMounted(() => {
  searchDepartmentComments();
});
</script>
<style lang="scss" scoped>
.department-comment-part {
  border: none;
