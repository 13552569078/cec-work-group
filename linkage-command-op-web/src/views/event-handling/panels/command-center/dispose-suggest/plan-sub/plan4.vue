...
import { RESOURCE_NAME } from '@/common';
...
const allRiskTypeList = computed(() => {
  return ALL_RESOURCES_TREE_NEW.value?.find(el => el.id === RESOURCE_NAME)?.children.map(i => { return { label: i.label, value: i.id }; });
});
const intelligenceResourceTypeList = computed(() => {
  return allConfigResourceType.value?.find(el => el.id === RESOURCE_NAME)?.children;
});
