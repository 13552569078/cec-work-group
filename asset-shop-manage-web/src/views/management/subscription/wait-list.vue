    ...
    <div class="page-wrap">
        ...
        <template v-slot:header="{ searchForm }">
          <c-form :model="searchForm" label-width="70px" inline>
            ...
            <c-form-item label="申请类型" class="lp-search-form-field">
              ...
              >
                <c-option
                  v-for="(item, k) in ASEET_APPLY_TYPE"
                  :key="k"
                  :label="item"
                  :value="k"
                >
            ...
            >
              <c-date-picker
                v-model="searchForm.applyHandleTime"
                style="width: 400px"
                type="datetimerange"
                range-separator="-"
                start-placeholder="开始日期"
      ...
      >
        ...
        <c-table :data="dataList" :height="tableHeight">
          ...
          >
            <template slot-scope="scope">
              {{ _applyTypeFormat(scope.row.applyType) }}
            </template>
          </c-table-column>
          ...
          >
            <template slot-scope="scope">
              {{ _applyStatusFormat(scope.row.applyStatus) }}
            </template>
          </c-table-column>
          <c-table-column
            prop="applyUserName"
...
import postAudit from '../components/post-audit-shop.vue'

import { applyAuditList, packAsyncApi } from '@/api/manage'
import {
  applyOptionsArr,
  applyTypeOptionsArr,
  applyObjectFormat,
  assetApplyTypeFormat,
  applyStatusOptionsArr,
  applyStatusFormat,
  ASSET_SOURCE,
  asseTypeFormat
} from '@/utils/enumerate'
import { ASEET_APPLY_TYPE } from '@/utils/enumerate'
...
export default {
  ...
  data() {
    return {
      myPath: '/management/sub-waiting-list',
      routesArr: [
  ...
  methods: {
    ...
    _applyTypeFormat(val) {
      return assetApplyTypeFormat(val)
    },
    ...
    getValName(item) {
      ...
      if (item.key === 'applyObject') {
        ...
        return assetApplyTypeFormat(item.value)
    ...
    handleDetail(row) {
      let path = '/management/sub-waiting-list/detail'
      if (this.pageStatus === 0) {
        path = '/management/sub-waiting-list/detailing'
      }
      this.$router.push({
        path,
        query: {
    ...
    createQueryParams(val) {
      ...
      // 审核状态
      if (this.pageStatus === 1) {
        data.applyStatusList = data.applyStatusList ? [data.applyStatusList] : [3, 4]
      } else if (this.pageStatus === 0) {
        data.applyStatusList = [1]
      }
    ...
    async search(pageNum = 1) {
      ...
      const res = await applyAuditList(this.createQueryParams(queryParams))
