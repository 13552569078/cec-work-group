      ...
      <template v-slot:table>
        <div>
          ...
          >
            ...
            <c-table-column fixed="right" label="操作" width="130">
              <template slot-scope="scope">
                <btn-group
...
export default {
  name: '',
  components: { treeLayout, postAudit },
  filters: {
    applyTypeFilter(val) {
      let res = ''
      switch (val) {
  ...
  data() {
    ...
    return {
      ...
      options: [
        ...
        { key: 'applyStatus', value: applyStatusList.map(item => ({ groupId: item.value, groupName: item.label })) },
  ...
  methods: {
    ...
    editData(row, flag) {
      if (row.applyObject + '' === '1') { // 资源
        this.$router.push({
          path: '/myApproval/wating/assetDetail',
          query: {
            applyId: row.id,
        ...
        this.$router.push({
          path: '/myApproval/wating/appDetail',
          query: {
            applyId: row.id,
            shopCode: row.shopCode,
            packVersion: row.packVersion,
    ...
    async getList() {
      ...
      try {
        ...
        queryParams.applyStatusList = [0]
        queryParams.orderKey = 'applyTime'
