    ...
    <div class="bg-white">
      ...
      <c-collapse v-show="list.length" v-model="collapseActive" accordion class="cls-collapse" @change="collapseChange">
        <c-collapse-item v-for="(item,i) in list" :key="item.packVersion" :name="i">
          <template slot="title">
            <div class="flex">
              <span class="title">{{ item.packVersion }}</span>
          ...
          <div class="detail">
            <div class="row-item">
              ...
              <span>{{ detail.packVersionDesc || '--' }}</span>
            </div>
            <div class="row-item">
              <label class="label">版本详情</label>
              <span class="show-snapshoot" @click="showSnapshoot(item)">查看快照</span>
            </div>
            <c-table :data="detailList" :border="true">
              <c-table-column prop="packStatus" label="上架状态" show-overflow-tooltip width="180">
...
import { queryVersionLog, queryVersionLogStatus } from '@/api/index'
...
export default {
  ...
  data() {
    const assetType = this.$route.path.indexOf('supportAppConfig') === -1 ? 22 : 27
    return {
      collapseActive: '',
      list: [],
      detail: {},
