...
<template>
  <div class="anchor">
    <i class="start"></i>
    <ul>
      <div class="title item" :class="{active:list[0].value==active}" @click="clickItem(list[0])">{{ list[0].label }}</div>
      <li
        v-for="d in list[0].children"
        :key="d.value"
        class="item"
        :class="{active:d.value==active}"
        @click="clickItem(d)">{{ d.label }}</li>
    </ul>
    <ul v-show="!hidepulick">
      <div class="title item" :class="{active:list[1].value==active}" @click="clickItem(list[1])">{{ list[1].label }}</div>
      <li
        v-for="d in list[1].children"
        :key="d.value"
        class="item"
        :class="{active:d.value==active}"
        @click="clickItem(d)">{{ d.label }}</li>
...
export default {
  ...
  data() {
    return {
      active: 'register',
      list: [
        { label: '注册信息', value: 'register', children: [
          { label: '基础信息', value: 'baseInfo' },
          { label: '项目信息', value: 'projectInfo' },
          { label: '厂商信息', value: 'firmInfo' }
        ] },
        { label: '上架信息', value: 'public', children: [
          { label: '版本信息', value: 'versionInfo' },
          { label: '安全信息', value: 'securityInfo' },
          { label: '案例指导', value: 'caseGuide' }
        ] }
      ],
      anchorDomList: []
    }
  ...
  methods: {
    ...
    handlePageScroll(e) {
      ...
      this.active = currentSection && currentSection.dataset.section || 'baseInfo'
    }
  }
}
</script>
