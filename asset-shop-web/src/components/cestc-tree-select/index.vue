...
<template>
  <c-select ref="selectDefault"
            ...
            :size="size"
            :multiple="multiple"
            :filterable="search&&!multiple&&allowCreate"
  ...
  >
    <c-option value="">
      <div class="bg-fff">
        ...
        >
          <!--
            与上面的:filterable="search&&!multiple&&allowCreate"互斥
            为了支持输入自定义的数据作为值
            加入multiple是因为多选选时和filter有问题
           -->
          <c-input
            v-if="!(search&&!multiple&&allowCreate)"
            v-model="filterText"
            :size="size"
            clearable
            placeholder="请输入"
        ...
        <div class="input-div" @click="clickCustom">{{ filterText }}</div>
        <c-tree
          id="tree-option"
          ref="selectTree"
...
export default {
  ...
  methods: {
    ...
    clickCustom() {
      this.valueTitle = this.filterText
      console.log('$emit----4')
      this.$emit('input', this.filterText)
      this.$emit('getValue', this.filterText)
    },
    ...
    filterNode(value, data) {
      ...
      const result = data[this.props.label] && data[this.props.label].indexOf(value) !== -1
      return result
    },
    initValue() {
    ...
    initHandle() {
      ...
      this.$nextTick(() => {
        ...
        scrollWrap && scrollWrap.forEach(ele => { ele.style.cssText = 'margin: 0px; max-height: none; overflow: hidden;' })
        scrollBar && scrollBar.forEach(ele => (ele.style.width = 0))
      })
    },
    getSelectedTitle(arr) {
      ...
      return tempArr.map(item => item.label)
    },
    // 切换选项
    ...
    clearSelected() {
      ...
      allNode && allNode.forEach(element => element.classList.remove('is-current'))
    }
  }
}
</script>
